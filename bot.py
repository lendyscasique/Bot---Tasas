# GSA CAMBIOS BOT — V1.0.6 / TRM + BCV robustos
"""
...
def get_trm_info():
    """TRM robusta.
    1) Superintendencia Financiera (oficial).
    2) Banco de la República (oficial).
    3) Último valor válido persistido.
    """
    ...
    try:
        ...
        r.raise_for_status()
        html = r.text
        # Ejemplo visible: TRM | COP | 3,126.08 | 5/Sep/2026 - 8/Sep/2026
        m = re.search(
            r"TRM.{0,300}?COP.{0,300}?([0-9]{1,3}(?:,[0-9]{3})*\.[0-9]{2}).{0,300}?([0-9]{1,2}/[A-Za-z]{3}/[0-9]{4}\s*-\s*[0-9]{1,2}/[A-Za-z]{3}/[0-9]{4})",
            html, re.I | re.S
        )
        if not m:
            # Variante por si el HTML viene con separadores distintos.
            m_val = re.search(r"TRM.{0,700}?([0-9]{1,3}(?:,[0-9]{3})*\.[0-9]{2})", html, re.I | re.S)
            m_vig = re.search(r"([0-9]{1,2}/[A-Za-z]{3}/[0-9]{4}\s*-\s*[0-9]{1,2}/[A-Za-z]{3}/[0-9]{4})", html, re.I)
            if m_val:
                valor = _numero_latam(m_val.group(1))
                vigencia = m_vig.group(1) if m_vig else ""
                if valor and 1000 < valor < 10000:
                    info = {"valor": valor, "fecha": vigencia, "fuente": "Superfinanciera", "respaldo": False}
    ...
    # Fuente 2: Banco de la República
    try:
        ...
        # Busca un valor con formato colombiano cerca de TRM.
        candidatos = re.findall(r"([0-9]{1,2}\.[0-9]{3},[0-9]{2})", html)
        valor = None
        for c in candidatos:
            v = _numero_latam(c)
            if v and 1000 < v < 10000:
                valor = v
                break

        # Fecha dd/mm/yyyy si está disponible cerca del indicador.
        fecha = ""
        mf = re.search(r"([0-3]?\d/[01]?\d/[0-9]{4})", html)
        if mf:
            fecha = mf.group(1)

        if valor:
            info = {"valor": valor, "fecha": fecha, "fuente": "Banco de la República", "respaldo": False}
    ...
    if ultimo.get("valor"):
        ultimo["fuente"] = (ultimo.get("fuente") or "Último valor válido") + " · respaldo"
        return ultimo

    return {"valor": None, "fecha": "", "fuente": "No disponible", "respaldo": True}
...
def get_bcv_info():
    """BCV robusto.
    1) Portal oficial BCV.
    2) DolarAPI (respaldo que replica BCV).
    3) Mirror de contenido BCV.
    4) Últimos valores válidos persistidos.
    """
    headers = {"User-Agent": "Mozilla/5.0 GSA-Cambios/1.0"}

    # Fuente 1: portal oficial
    try:
        r = requests.get("https://www.bcv.org.ve/", headers=headers, timeout=15, verify=False)
        r.raise_for_status()
        usd, eur, fecha = _extraer_bcv_html(r.text)
        if usd and eur:
            _guardar_tasa_config("bcv_usd", usd, fecha, "BCV")
            _guardar_tasa_config("bcv_eur", eur, fecha, "BCV")
            return {
                "usd": usd, "eur": eur, "fecha": fecha,
                "fuente": "BCV", "respaldo": False
            }
    except Exception as e:
    ...
    try:
        ...
        for item in data if isinstance(data, list) else []:
            ...
            fecha = fecha or str(item.get("fechaActualizacion") or item.get("fecha") or "")
        if usd and eur:
            _guardar_tasa_config("bcv_usd", usd, fecha, "DolarAPI/BCV")
            _guardar_tasa_config("bcv_eur", eur, fecha, "DolarAPI/BCV")
            return {
                "usd": usd, "eur": eur, "fecha": fecha,
                "fuente": "DolarAPI/BCV", "respaldo": True
            }
    except Exception as e:
        print(f"[BCV] DolarAPI falló: {e}")

    # Fuente 3: mirror del contenido público BCV
    try:
        r = requests.get(
            "https://cablepar.com.ve/proxy_dolar.php",
            headers=headers, timeout=12
        )
        r.raise_for_status()
        usd, eur, fecha = _extraer_bcv_html(r.text)
        if usd and eur:
            _guardar_tasa_config("bcv_usd", usd, fecha, "Mirror BCV")
            _guardar_tasa_config("bcv_eur", eur, fecha, "Mirror BCV")
            return {
                "usd": usd, "eur": eur, "fecha": fecha,
                "fuente": "Mirror BCV", "respaldo": True
            }
    except Exception as e:
        print(f"[BCV] mirror falló: {e}")

    # Últimos valores persistidos
    usd_prev = _leer_tasa_config("bcv_usd")
    eur_prev = _leer_tasa_config("bcv_eur")
    if usd_prev.get("valor") or eur_prev.get("valor"):
        return {
            "usd": usd_prev.get("valor"),
            "eur": eur_prev.get("valor"),
            "fecha": usd_prev.get("fecha") or eur_prev.get("fecha") or "",
            "fuente": "Último BCV válido · respaldo",
            "respaldo": True,
        }

    return {
        "usd": None, "eur": None, "fecha": "",
...
def get_bcv():
    info = get_bcv_info()
    return info.get("usd"), info.get("eur")
...
def consultar_y_guardar(western_rate=None):
    ...
    bcv_usd = bcv_info.get('usd')
    bcv_eur = bcv_info.get('eur')
    ...
    datos = {
        ...
        'bcv_fecha':bcv_info.get('fecha'),'bcv_fuente':bcv_info.get('fuente'),
        'bcv_respaldo':bcv_info.get('respaldo',False),
        'trm_fecha':trm_info.get('fecha'),'trm_fuente':trm_info.get('fuente'),
    ...
    # Guardar en DB
    if USE_SUPABASE:
        _meta_no_db = {'mejor_banco','bcv_fecha','bcv_fuente','bcv_respaldo','trm_fecha','trm_fuente','trm_respaldo'}
