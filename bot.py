...
def construir_mensaje_tasas_v1(d, es_especial=False):
    ...
    2) tasas/políticas comerciales GSA,
    3) operaciones que requieren dato manual o guardrail.

    No usa los rates comerciales legacy guardados en tasa_gsa_* para
    formar el panel V1.
    """
    ahora = now_local().strftime("%d/%m/%Y — %I:%M %p")
    ban_s = float(d.get('ban_bs_spread', 0) or 0)
    mer_s = float(d.get('mer_bs_spread', 0) or 0)
    mejor = d.get('mejor_banco', '—')
    prefijo = "🔔 *TASA DE REFERENCIA*\n" if es_especial else ""

    m = f"{prefijo}📊 *GSA CAMBIOS — MERCADO Y TASAS V1*\n"
    m += f"📅 {ahora}\n"
    m += "━━━━━━━━━━━━━━━━━━━━\n\n"

    # ── 1. REFERENCIAS DE MERCADO ───────────────────────────────────
    m += "🌎 *REFERENCIAS DE MERCADO*\n"
    m += "_Informativas; no todas son tasas comerciales GSA._\n\n"

    if d.get('trm'):
        m += f"🇨🇴 TRM: `{fmt(d['trm'])} COP/USD`\n"
    if d.get('bcv_usd'):
        m += f"🇻🇪 BCV USD: `{fmt(d['bcv_usd'])} Bs/USD`\n"
    if d.get('dolar_obs'):
        m += f"🇨🇱 Dólar Observado: `{fmt(d['dolar_obs'])} CLP/USD`\n"
    if d.get('western'):
        m += f"🌍 Western: `{fmt(d['western'],4)} COP/CLP`\n"
    else:
        m += "🌍 Western: `_N/D — usa /western TASA_`\n"

    if d.get('clp_compra') or d.get('clp_venta'):
        m += (
            f"🔵 Binance USDT/CLP: "
            f"`{fmt(d.get('clp_compra'))} / {fmt(d.get('clp_venta'))}`\n"
        )
    if d.get('cop_compra') or d.get('cop_venta'):
        m += (
            f"🔵 Binance USDT/COP: "
            f"`{fmt(d.get('cop_compra'))} / {fmt(d.get('cop_venta'))}`\n"
        )
    if d.get('ban_bs_compra') or d.get('ban_bs_venta'):
        m += (
            f"🏦 Banesco USDT/BS: "
            f"`{fmt(d.get('ban_bs_compra'))} / {fmt(d.get('ban_bs_venta'))}` "
            f"{spread_emoji(ban_s)} spread `{fmt(ban_s)}`\n"
        )
    if d.get('mer_bs_compra') or d.get('mer_bs_venta'):
        m += (
            f"🏦 Mercantil USDT/BS: "
            f"`{fmt(d.get('mer_bs_compra'))} / {fmt(d.get('mer_bs_venta'))}` "
            f"{spread_emoji(mer_s)} spread `{fmt(mer_s)}`\n"
        )
    m += f"\n⭐ Mercado BS preferido por spread: *{mejor}*\n"

    # BS→COP automática: SOLO referencia, nunca tasa negociada.
    bscop_ref = float(d.get('limite_bs_cop', 0) or 0)
    if bscop_ref > 0:
        m += f"🇻🇪→🇨🇴 Ref. automática BS→COP: `{bscop_ref:.4f} COP/Bs`\n"
        m += "_No sustituye la tasa real negociada de adquisición COP._\n"

    # ── 2. TASAS/POLÍTICAS COMERCIALES DEFINITIVAS ─────────────────
    m += "\n━━━━━━━━━━━━━━━━━━━━\n"
    m += "💱 *TASAS COMERCIALES GSA — V1*\n\n"

    # #1 CLP→BS: AUTO = 5%. La capacidad usa la ruta económica actual
    # configurada en el Bot. Si falta mercado, no se publica.
    try:
        capacidad_50k = _capacidad_clp_bs_actual(d, 50000.0)
        q1 = cotizar_clp_bs(50000.0, capacidad_50k, margen="AUTO")
        if q1.get("tasa_comercial") and q1.get("estado") != ESTADO_BLOQUEADO:
            m += f"🇨🇱→🇻🇪 CLP → Bs (AUTO 5%): `{q1['tasa_comercial']:.6f}`\n"
    except Exception:
        m += "🇨🇱→🇻🇪 CLP → Bs (AUTO 5%): `N/D`\n"

    # #3 CLP→COP: tabla comercial aprobada; la autorización se hace
    # contra capacidad real/tasa BS→COP negociada en /simular.
    m += "\n🇨🇱→🇨🇴 *CLP → COP — tabla comercial vigente*\n"
    m += "• `0 – 25.000 CLP` → `3.00`\n"
    m += "• `25.001 – 50.000` → `3.05`\n"
    m += "• `50.001 – 150.000` → `3.10`\n"
    m += "• `150.001+` → `3.15`\n"
    m += "_La tabla no autoriza por sí sola: /simular valida capacidad, margen y tasa BS→COP real._\n"

    # #4 COP→CLP: Western vs piso de ciclo usando Smax 3.15 y objetivo 5%.
    if d.get('western'):
        try:
            q4 = cotizar_cop_clp(500000.0, float(d['western']), 3.15)
            m += f"\n🇨🇴→🇨🇱 COP → CLP: `{q4['tasa_comercial']:.4f} COP/CLP`\n"
            if q4.get("alertas"):
                m += "_Western está bajo el piso rentable; se usa el piso del ciclo._\n"
        except Exception:
            m += "\n🇨🇴→🇨🇱 COP → CLP: `N/D`\n"

    # #5–6 USD físico Chile
    if d.get('dolar_obs'):
        try:
            q5 = cotizar_clp_usd(100000.0, float(d['dolar_obs']))
            q6 = cotizar_usd_clp(100.0, float(d['dolar_obs']))
            m += f"🇨🇱→🇺🇸 CLP → USD físico: `{q5['tasa_comercial']:.2f} CLP/USD`\n"
            m += f"🇺🇸→🇨🇱 USD físico → CLP: `{q6['tasa_comercial']:.2f} CLP/USD`\n"
        except Exception:
            pass

    # #7–8 Directo Binance. /tasas solo dispone de la referencia automática
    # de mercado; maker propio se compara dentro de /simular cuando se informa.
    try:
        clp_buy = float(d.get('clp_compra', 0) or 0)
        if clp_buy > 0:
            q7 = cotizar_clp_usdt(
                100000.0,
                precio_taker_neto=clp_buy,
                taker_disponible=True,
                maker_disponible=False,
                modo="AUTO",
            )
            if q7.get("estado") != ESTADO_BLOQUEADO:
                m += f"🇨🇱→💎 CLP → USDT (taker ref.): `{q7['tasa_comercial']:.2f} CLP/USDT`\n"
    except Exception:
        pass

    try:
        clp_sell = float(d.get('clp_venta', 0) or 0)
        if clp_sell > 0:
            q8 = cotizar_usdt_clp(
                100.0,
                capacidad_taker=clp_sell,
                taker_disponible=True,
                maker_disponible=False,
                modo="AUTO",
            )
            if q8.get("estado") != ESTADO_BLOQUEADO:
                m += f"💎→🇨🇱 USDT → CLP (taker ref.): `{q8['tasa_comercial']:.2f} CLP/USDT`\n"
    except Exception:
        pass

    # #9 Western
    m += "🌍 Western → COP: `cliente recibe 95% del nominal`\n"

    # ── 3. PROVISIONALES / REQUIEREN GUARDRAIL ─────────────────────
    m += "\n━━━━━━━━━━━━━━━━━━━━\n"
    m += "🧭 *COTIZACIÓN BAJO SIMULACIÓN*\n"
    m += "_No se publican como tasa automática sin validar ejecución/inventario._\n\n"

    m += "• `BS → CLP` — política provisional\n"
    m += "• `COP → BS` — política provisional\n"
    m += "• `BS → COP` — compara Binance vs tasa directa real\n"

    # #12 referencia comercial, no autorización.
    bs_ref = max(
        float(d.get('ban_bs_venta', 0) or 0),
        float(d.get('mer_bs_venta', 0) or 0),
    )
    if bs_ref > 0:
        m += f"• `USD físico → BS` — referencia comercial `{bs_ref - 30:.2f} Bs/USD`, requiere valor económico\n"
    else:
        m += "• `USD físico → BS` — requiere referencia Binance + valor económico\n"
    m += "• `BS → USD físico` — requiere costo de reposición/inventario\n"
    m += "• `USD físico ↔ COP` — requiere mercado físico Cúcuta compra/venta\n"

    # #16 provisional direct Binance
    cop_sell = float(d.get('cop_venta', 0) or 0)
    if cop_sell > 0:
        try:
            q16 = cotizar_usdt_cop(
                100.0,
                capacidad_taker=cop_sell,
                taker_disponible=True,
                maker_disponible=False,
                modo="AUTO",
            )
            m += f"• `USDT → COP` — provisional 1%: `{q16['tasa_comercial']:.2f} COP/USDT`\n"
        except Exception:
            m += "• `USDT → COP` — política provisional 1%\n"
    else:
        m += "• `USDT → COP` — política provisional 1%\n"

    m += "• `USDT → USD físico` — 2,5% bruto + guardrail de reposición\n"
    m += "• `USD físico → USDT` — 1:1 solo cuando conviene captar efectivo\n"

    m += "\n━━━━━━━━━━━━━━━━━━━━\n"
    m += "🧮 Usa `/simular` para autorizar una operación concreta.\n"
    m += "_Capacidad primero → precio → utilidad → guardrail._"
    return m
...
def calcular_simulacion_motor_v1(tipo_op, monto, campos=None, tasas=None):
    ...
    if tipo == "BS→CLP":
        # Provisional: BS→USDT→CLP con referencias automáticas disponibles.
        if bs_buy <= 0 or clp_sell <= 0:
            raise ValueError("Sin mercado suficiente BS→USDT→CLP")
        usdt = monto / bs_buy
        capacidad = usdt * clp_sell
        return cotizar_bs_clp(monto, capacidad)
