# GSA CAMBIOS BOT — V1 SIMPLE / Pricing CLP→COP
"""
            ...
            try:
                ...
                if tipo_op not in TIPOS_OP:
                    ...
                    resultado, error = calcular_cotizacion_clp_cop_v1(
                        ...
                        otros_cop=otros_cop, notas=notas)
                    if error:
...
def msg_limites_clp_cop_v1():
    ...
    etiquetas = ["Hasta 50.000 CLP", "50.001–100.000 CLP", "Más de 100.000 CLP"]
    ...
    for et, ref, f in zip(etiquetas, referencias, filas):
        ...
        m += f"🔴 Máx. con utilidad mínima: `{f['max']:.4f}`\n\n"
...
def msg_simular_ayuda():
    ...
    return (
        ...
        "`OTROS: 0`\n\n"
        "*Cómo llenar:*\n"
        "• `REFERIDO`: SI o NO. El Bot aplica el 3% automáticamente.\n"
        ...
        "El Bot calcula mercado, fees, tasa cliente y la tasa BS→COP mínima necesaria.\n"
        "_Por ahora el nuevo motor especial está validado para CLP→COP. Las demás rutas conservan su lógica actual hasta el motor universal._"
    )
...
def calcular_cotizacion_clp_cop_v1(
    ...
    avance_corresponsal_cop=0.0, otros_cop=0.0, notas='-'
):
    ...
    # Objetivo BS→COP: tasa mínima para sostener la cotización recomendada,
    # respetando 7% o la utilidad mínima absoluta, lo que sea más exigente.
    cop_cliente_obj = float(monto_clp) * float(tasa_eval)
    margen_piso = max(MARGEN_RECOMENDADO, (UTILIDAD_MINIMA_COP / cop_cliente_obj) if cop_cliente_obj > 0 else MARGEN_RECOMENDADO)
    q_objetivo_min = calcular_bs_cop_requerida(
        ciclo['bs_netos'], float(monto_clp), tasa_eval, margen_piso,
        referido=referido_activo, delivery_cop=costos_fijos_cop,
        avance_corresponsal_cop=avance_corresponsal_cop,
    )
    ...
    return {
        ...
        'evaluacion':evaluacion, 'western_cmp':comparacion, 'bs_cop_requerida':inverso, 'q_objetivo_min':q_objetivo_min, 'margen_piso':margen_piso,
    }, None
...
def msg_cotizacion_clp_cop_v1(r):
    ...
    m += "*COTIZACIÓN RECOMENDADA*\n"
    m += f"Tasa cliente: `{r['tasa_cliente']:.4f} COP/CLP`\n"
    m += f"Cliente recibe: `{ev['cop_cliente']:,.0f} COP`\n"
    m += f"Utilidad estimada: `{ev['utilidad_cop']:,.0f} COP`\n"
    m += f"Rentabilidad: `{ev['rentabilidad_pct']:.2f}%`\n"
    m += f"Estado: *{etiquetas.get(ev['clasificacion'], ev['clasificacion'])}*\n\n"

    m += "*NEGOCIACIÓN*\n"
    m += f"🟢 7% recomendada: `{niv['recomendada']['tasa']:.4f}`\n"
    m += f"🔵 5% preferencial: `{niv['preferencial']['tasa']:.4f}`\n"
    m += f"🟠 3% captación: `{niv['captacion']['tasa']:.4f}`\n"
    m += f"⚫ Equilibrio: `{niv['equilibrio']['tasa']:.4f}`\n\n"

    m += "*COMPRA BS→COP*\n"
    m += f"Referencia mercado: `{q_ref:.4f} COP/Bs`\n"
    m += f"Objetivo mínimo para esta cotización: `{q_obj:.4f} COP/Bs`\n"
    if diferencia_q >= 0:
        m += f"✅ La referencia actual cubre el objetivo por `{diferencia_q:.4f}`.\n"
        m += "_Si consigues una tasa BS→COP mayor, mejoras la rentabilidad._"
    else:
        m += f"⚠️ Necesitas negociar al menos `{abs(diferencia_q):.4f}` COP/Bs por encima de la referencia actual."
