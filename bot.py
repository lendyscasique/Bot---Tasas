...
# ══════════════════════════════════════════════════════════════════════
def get_top_anuncios_bs(min_usdt=0):
    """Retorna top anuncios BS. min_usdt=0 para referencia, 1000 para Maker operable."""
...
def get_mercado_bs_completo():
    """Retorna referencia (todos) y Maker operable (≥1000 USDT) por separado."""
    ref_compras, ref_ventas     = get_top_anuncios_bs(min_usdt=0)
    maker_compras, maker_ventas = get_top_anuncios_bs(min_usdt=1000)
    return {
...
def get_binance_banco_promedio(banco):
    ...
    # Compra: solo el banco específico
    # Venta: banco + PagoMovil para capturar mejores precios
...
def msg_sesion_status(chat_id):
    ...
    s = sesion_activa[chat_id]
    compras_bs, ventas_bs = get_top_anuncios_bs(min_usdt=1000)
...
def msg_mercado_completo():
    ...
    if mk_c:
        ...
        for i, a in enumerate(mk_c, 1):
            m += f"    {i}️⃣ `{a['usuario']:15s}` `{a['precio']:,.2f} Bs` | `{a['disponible']:,.0f} USDT`\n"
    else:
        m += "  📥 Sin anuncios compra ≥1,000 USDT\n"
    if mk_v:
        m += "  📤 Venta:\n"
        for i, a in enumerate(mk_v, 1):
            m += f"    {i}️⃣ `{a['usuario']:15s}` `{a['precio']:,.2f} Bs` | `{a['disponible']:,.0f} USDT`\n"
    else:
        m += "  📤 Sin anuncios venta ≥1,000 USDT\n"
    if mk_c and mk_v:
...
def verificar_capital_operacion(datos_op):
    ...
    if mon_entrada == 'CLP':
        ...
        mk_c, mk_v = get_top_anuncios_bs(min_usdt=1000)
        if mk_c and mk_v:
