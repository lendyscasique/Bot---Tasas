# GSA CAMBIOS BOT — V1.0.5 / Pricing CLP→COP + Política de Margen
"""
...
from pricing_gsa import (
    ...
    # Motor universal V1
    cotizar_clp_bs, cotizar_bs_clp, cotizar_clp_cop, cotizar_cop_clp,
    cotizar_clp_usd, cotizar_usd_clp, cotizar_clp_usdt, cotizar_usdt_clp,
    cotizar_western_cop, cotizar_cop_bs, cotizar_bs_cop, cotizar_usd_bs,
    cotizar_bs_usd, cotizar_usd_cop, cotizar_cop_usd, cotizar_usdt_cop,
    cotizar_usdt_usd, cotizar_usd_usdt,
    ESTADO_BLOQUEADO, ESTADO_RENTABLE, ESTADO_MARGEN_COMPRIMIDO,
    ESTADO_CAPTACION, ESTADO_ESTRATEGICA, ESTADO_NO_RECOMENDADO,
    POLICY_PROVISIONAL,
    ALERTA_MARGEN_COMPRIMIDO, ALERTA_CAPTACION, ALERTA_POLITICA_PROVISIONAL,
    ALERTA_BANESCO_003, ALERTA_MAKER_NO_DISPONIBLE, ALERTA_TAKER_NO_DISPONIBLE,
    ALERTA_COSTO_REPOSICION, ALERTA_INVENTARIO, ALERTA_WESTERN_BAJO_UMBRAL,
    ALERTA_MERCADO_FISICO_INVALIDO, ALERTA_DATO_INSUFICIENTE,
    ALERTA_LIQUIDEZ_INSUFICIENTE, ALERTA_OPERACION_NO_RENTABLE,
    ALERTA_INPUT_INVALIDO, ALERTA_SPREAD_PROVISIONAL_1,
)
...
def msg_mercado_completo():
    """Lectura interna de mercado. VES usa referencia ejecutable a 1.000 Bs."""
    ban_c, ban_v, ban_s = get_binance_banco_promedio("Banesco")
    compras_clp, ventas_clp = get_top_anuncios_clp()
    cop_vende, cop_compra = get_binance_fiat_promedio("COP")
    ahora = now_local().strftime("%d/%m/%Y — %I:%M %p")

    m = f"📡 *GSA CAMBIOS — MERCADO EN VIVO*\n📅 {ahora}\n━━━━━━━━━━━━━━━━━━━━\n\n"

    m += "🇻🇪 *BINANCE P2P — BS*\n\n"
    m += "Referencia TAKER\n"
    m += "Filtro de operación: *1.000 Bs*\n\n"
    m += "Bs → USDT\nLado Binance: *VENDE*\n"
    m += f"• Ejecutable Banesco: `{ban_c:,.2f} Bs/USDT`\n\n" if ban_c else "• Ejecutable Banesco: `N/D`\n\n"
    m += "USDT → Bs\nLado Binance: *COMPRA*\n"
    m += f"• Ejecutable Banesco: `{ban_v:,.2f} Bs/USDT`\n" if ban_v else "• Ejecutable Banesco: `N/D`\n"
    if ban_s:
        m += f"\nSpread Banesco: `{ban_s:,.2f} Bs`\n"

    m += "\n━━━━━━━━━━━━━━━━━━━━\n🇨🇱 *BINANCE P2P — CLP*\n\n"
    m += "CLP → USDT\nLado Binance: *VENDE*\n"
    if compras_clp:
        m += f"• Mejor ejecutable: `{compras_clp[0]['precio']:,.2f} CLP/USDT`\n\n"
    else:
        m += "• Mejor ejecutable: `N/D`\n\n"
    m += "USDT → CLP\nLado Binance: *COMPRA*\n"
    if ventas_clp:
        m += f"• Mejor ejecutable: `{ventas_clp[0]['precio']:,.2f} CLP/USDT`\n"
    else:
        m += "• Mejor ejecutable: `N/D`\n"

    m += "\n━━━━━━━━━━━━━━━━━━━━\n🇨🇴 *BINANCE P2P — COP*\n\n"
    m += "COP → USDT\nLado Binance: *VENDE*\n"
    m += f"• Referencia: `{cop_vende:,.2f} COP/USDT`\n\n" if cop_vende else "• Referencia: `N/D`\n\n"
    m += "USDT → COP\nLado Binance: *COMPRA*\n"
    m += f"• Referencia: `{cop_compra:,.2f} COP/USDT`\n" if cop_compra else "• Referencia: `N/D`\n"

    m += "\n━━━━━━━━━━━━━━━━━━━━\n📌 _Información operativa interna._"
    return m


def generar_reporte_diario():
    """Análisis de mercado del día, sin resultados contables."""
    hoy = str(today_local())
    conn = get_conn()
    rows = conn.execute("""
    ...
    m = f"📊 *ANÁLISIS DEL MERCADO — HOY*\n📅 {now_local().strftime('%d/%m/%Y')}\n"
    m += "━━━━━━━━━━━━━━━━━━━━\n\n"
    m += "🇻🇪 *USDT / BS*\n\n"
    m += f"Precio más alto: `{max_venta[1]:.2f} Bs/USDT` — `{max_venta[0]}`\n"
    m += f"Precio más bajo: `{min_venta[1]:.2f} Bs/USDT` — `{min_venta[0]}`\n"
    m += f"Promedio del día: `{prom_venta:.2f} Bs/USDT`\n\n"
    m += "📊 *SPREAD*\n\n"
    m += f"Máximo: `{max_spread[1]:.2f} Bs` — `{max_spread[0]}`\n"
    m += f"Mínimo: `{min_spread[1]:.2f} Bs` — `{min_spread[0]}`\n"
    m += f"Promedio: `{prom_spread:.2f} Bs`\n\n"
    h = int(mejor_ventana[0])
    m += f"🏆 *Mejor ventana del día*\n`{h:02d}:00 – {(h+1)%24:02d}:00`\n\n"
    m += "📌 _Análisis basado en las lecturas almacenadas durante el día._"
    return m
...
def consultar_y_guardar(western_rate=None):
    # Western sigue siendo una referencia para los límites legacy/referenciales.
    # Si el proceso se reinicia, recuperamos la última tasa guardada en config.
    if not western_rate:
        try:
            western_guardado = float(get_config('western_rate', '0') or 0)
            western_rate = western_guardado if western_guardado > 0 else None
        except Exception:
            western_rate = None
    ...
    if (mer_s or 0) > 0 and (ban_s or 0) > 0:
        mejor = "Mercantil" if mer_s < ban_s else "Banesco"
    elif (mer_s or 0) > 0:
        mejor = "Mercantil"
    else:
        mejor = "Banesco"
...
def calcular_base_clp_cop_dinamica(d):
    """Base dinámica que reproduce el 3.00 original."""
    western = float(d.get("western", 0) or 0)
    if western <= 0:
        return 0.0
    limite_clp_cop = western * (1 - FEE_WU)
    return limite_clp_cop * (1 - FEE_CLP_COP)


def obtener_tramos_clp_cop(d, persistir=True):
    """Tramo 1 dinámico; siguientes +0.05.
    Baja intradía por protección y no sube hasta el día siguiente.
    La base publicada conserva 4 decimales para hacer visible cada actualización.
    """
    raw = calcular_base_clp_cop_dinamica(d)
    if raw <= 0:
        return {"base":0.0,"candidata":0.0,"estado":"SIN_DATO","tramos":[]}

    candidata = round(raw, 4)
    publicada = candidata
    estado = "NUEVA"
    ...
    if persistir:
        try:
            hoy = str(today_local())
            fecha = get_config("clp_cop_fecha_ancla", "")
            try:
                previa = float(get_config("clp_cop_base_publicada", "0") or 0)
            except Exception:
                previa = 0.0

            if fecha != hoy or previa <= 0:
                publicada = candidata
                set_config("clp_cop_fecha_ancla", hoy)
                set_config("clp_cop_base_publicada", f"{publicada:.4f}")
                estado = "APERTURA"
            elif candidata < previa:
                publicada = candidata
                set_config("clp_cop_base_publicada", f"{publicada:.4f}")
                estado = "BAJA_PROTECCION"
            else:
                publicada = round(previa, 4)
                estado = "MANTENER"
        except Exception:
            publicada = candidata
            estado = "SIN_PERSISTENCIA"
    ...
    return {
        ...
        "estado": estado,
        "tramos": [
            (25000, round(publicada,4)),
            (50000, round(publicada+0.05,4)),
            (150000, round(publicada+0.10,4)),
            (float("inf"), round(publicada+0.15,4)),
        ],
    }

def tasa_clp_cop_por_monto(d, monto_clp, persistir=True):
    info = obtener_tramos_clp_cop(d, persistir=persistir)
    for limite, tasa in info["tramos"]:
        if float(monto_clp) <= limite:
            return tasa
    return 0.0


def tasas_bs_cop_comerciales(d):
    """Pizarra comercial BS/COP aprobada por GSA.
    Compra Bs = tasa superior; Venta Bs = tasa inferior.
    """
    limite = float(d.get("limite_bs_cop", 0) or 0)
    if limite <= 0:
        return None
    return {
        "compra_bs": round(limite * (1 + FEE_BS_COP), 4),
        "venta_bs": round(limite * (1 - FEE_COP_BS), 4),
    }

def lados_binance_explicitamente(d):
    """Nombres operativos explícitos. Preserva campos legacy del payload."""
    return {
        "clp_vende": float(d.get("clp_compra", 0) or 0),   # CLP→USDT
        "clp_compra": float(d.get("clp_venta", 0) or 0),  # USDT→CLP
        "cop_vende": float(d.get("cop_compra", 0) or 0),  # COP→USDT
        "cop_compra": float(d.get("cop_venta", 0) or 0),  # USDT→COP
    }



def tasas_usd_cop_comerciales(d):
    """Referencia visible USD/COP.
    La pizarra comercial muestra Compra USD = menor y Venta USD = mayor.
    La orientación técnica Binance se conserva aparte:
    COP→USDT = VENDE; USDT→COP = COMPRA.
    """
    lado_vende = float(d.get("cop_compra", 0) or 0)  # COP→USDT
    lado_compra = float(d.get("cop_venta", 0) or 0)  # USDT→COP
    valores = [v for v in (lado_vende, lado_compra) if v > 0]
    if len(valores) < 2:
        return None
    return {
        "compra_usd_ref": round(min(valores), 2),
        "venta_usd_ref": round(max(valores), 2),
    }


def construir_mensaje_tasas_v1(d, es_especial=False):
    """Pizarra comercial V1.1.1 aprobada."""
    ahora = now_local().strftime("%d/%m/%Y — %I:%M %p")
    m = f"💱 *GSA CAMBIOS — TASAS*\n📅 {ahora}\n━━━━━━━━━━━━━━━━━━━━\n\n"

    limite_clp_bs = float(d.get("limite_clp_bs", 0) or 0)

    m += "🇨🇱🇻🇪 *PESOS CHILENOS / BOLÍVARES*\n\n"
    try:
        capacidad = _capacidad_clp_bs_actual(d, 50000.0)
        q = cotizar_clp_bs(50000.0, capacidad, margen="AUTO")
        if q.get("estado") != ESTADO_BLOQUEADO:
            m += f"CLP → Bs\n• Tasa: `{q['tasa_comercial']:.4f} Bs/CLP`\n\n"
    except Exception:
        m += "CLP → Bs\n• Tasa: `N/D`\n\n"
    if limite_clp_bs > 0:
        m += f"Bs → CLP\n• Tasa: `{limite_clp_bs*1.10:.4f} Bs/CLP`\n"
    else:
        m += "Bs → CLP\n• Tasa: `N/D`\n"

    info = obtener_tramos_clp_cop(d, persistir=True)
    m += "\n\n🇨🇱🇨🇴 *PESOS CHILENOS / PESOS COLOMBIANOS*\n\n"
    if info["tramos"]:
        t1,t2,t3,t4 = [x[1] for x in info["tramos"]]
        m += "CLP → COP\n"
        m += f"• Hasta 25.000 CLP        → `{t1:.4f}`\n"
        m += f"• 25.001 – 50.000 CLP     → `{t2:.4f}`\n"
        m += f"• 50.001 – 150.000 CLP    → `{t3:.4f}`\n"
        m += f"• Desde 150.001 CLP       → `{t4:.4f}`\n\n"
    else:
        m += "CLP → COP\n• Tasa: `N/D`\n\n"

    if d.get("western"):
        try:
            smax = info["tramos"][-1][1] if info["tramos"] else 3.15
            q = cotizar_cop_clp(500000.0, float(d["western"]), smax)
            if q.get("estado") != ESTADO_BLOQUEADO:
                m += f"COP → CLP\n• Tasa: `{q['tasa_comercial']:.4f} COP/CLP`\n"
            else:
                m += "COP → CLP\n• Tasa: `N/D`\n"
        except Exception:
            m += "COP → CLP\n• Tasa: `N/D`\n"
    ...
    bs_cop = tasas_bs_cop_comerciales(d)
    if bs_cop:
        m += "\n\n🇻🇪🇨🇴 *BOLÍVARES / PESOS COLOMBIANOS*\n\n"
        m += f"• Compra Bs → `{bs_cop['compra_bs']:.4f} COP/Bs`\n"
        m += f"• Venta Bs  → `{bs_cop['venta_bs']:.4f} COP/Bs`\n"

    usd_cop = tasas_usd_cop_comerciales(d)
    if usd_cop:
        m += "\n\n💵🇨🇴 *USD / PESOS COLOMBIANOS*\n\n"
        m += f"• Compra USD → `{usd_cop['compra_usd_ref']:,.2f} COP/USD`\n"
        m += f"• Venta USD  → `{usd_cop['venta_usd_ref']:,.2f} COP/USD`\n"

    if d.get("dolar_obs"):
        obs = float(d["dolar_obs"])
        m += "\n\n💵🇨🇱 *USD / PESOS CHILENOS*\n\n"
        m += f"• Compra USD → `{obs-SPREAD_CLP:,.2f} CLP/USD`\n"
        m += f"• Venta USD  → `{obs+SPREAD_CLP:,.2f} CLP/USD`\n"

    clp_vende = float(d.get("clp_compra", 0) or 0)  # CLP→USDT
    clp_compra = float(d.get("clp_venta", 0) or 0) # USDT→CLP
    if clp_vende > 0 or clp_compra > 0:
        m += "\n\n💎🇨🇱 *USDT / PESOS CHILENOS*\n\n"
        if clp_compra > 0:
            m += f"• Compra USDT → `{clp_compra-20:,.2f} CLP/USDT`\n"
        if clp_vende > 0:
            m += f"• Venta USDT  → `{clp_vende+20:,.2f} CLP/USDT`\n"

    # BS visible: Banesco únicamente. Binance/Mercantil pueden seguir siendo fuentes internas.
    ban_compra = float(d.get("ban_bs_compra", 0) or 0) # Bs→USDT, lado VENDE
    ban_venta  = float(d.get("ban_bs_venta", 0) or 0)  # USDT→Bs, lado COMPRA

    if ban_venta > 0:
        m += "\n\n💵🇻🇪 *USD / BOLÍVARES*\n\n"
        m += f"• Compra USD → `{max(0.0, ban_venta-MARGEN_BS):,.2f} Bs/USD`\n"
        m += f"• Venta USD  → `{ban_venta:,.2f} Bs/USD`\n"

    if ban_compra > 0 or ban_venta > 0:
        m += "\n\n💎🇻🇪 *USDT / BOLÍVARES*\n\n"
        if ban_compra > 0:
            m += f"• Compra USDT → `{ban_compra:,.2f} Bs/USDT`\n"
        if ban_venta > 0:
            m += f"• Venta USDT  → `{ban_venta:,.2f} Bs/USDT`\n"

    m += "\n\n🌎 *WESTERN / CORRESPONSAL*\n\n"
    m += "Giros internacionales\n"
    m += "• Comisión GSA: *5%*\n"
    m += "• Se calcula sobre el monto recibido por el corresponsal.\n\n"
    m += "Ejemplo:\n"
    m += "Monto recibido: `500.000 COP`\n"
    m += "Comisión GSA 5%: `25.000 COP`\n"
    m += "Monto neto: `475.000 COP`\n"

    refs = []
    if d.get("trm"):
        refs.append(f"🇨🇴 TRM `{fmt(d['trm'])} COP/USD`")
    if d.get("dolar_obs"):
        refs.append(f"🇨🇱 Dólar Observado `{fmt(d['dolar_obs'])} CLP/USD`")
    if ban_compra > 0 or ban_venta > 0:
        partes = []
        if ban_compra > 0: partes.append(f"Compra {ban_compra:,.2f}")
        if ban_venta > 0: partes.append(f"Venta {ban_venta:,.2f}")
        refs.append("🇻🇪 Banesco `" + " · ".join(partes) + " Bs/USDT`")
    if d.get("bcv_usd"):
        refs.append(f"🏦 BCV USD `{fmt(d['bcv_usd'])} Bs/USD`")
    if d.get("bcv_eur"):
        refs.append(f"🇪🇺 BCV EUR `{fmt(d['bcv_eur'])} Bs/EUR`")
    if d.get("western"):
        refs.append(f"🌎 Western `{fmt(d['western'],4)} COP/CLP`")
    ...
    m += "\n\n━━━━━━━━━━━━━━━━━━━━\n"
    m += "⚠️ Tasas sujetas a disponibilidad y condiciones del mercado al momento de confirmar la operación.\n\n"
    m += "*GSA CAMBIOS*"
    return m
...
def construir_mensaje(d, es_especial=False):
    ...
    if d.get('western'):     m += f"🌍  *Western Unión*\n      `{fmt(d['western'],4)} COP/CLP`\n\n"
    else:                    m += f"🌍  *Western Unión*\n      _Envía /western TASA_\n\n"

    # Referencia BS→COP automática: se deriva del límite referencial ya calculado.
    # No es una tasa negociada ni una tasa fija de GSA.
    bscop_ref = float(d.get('limite_bs_cop', 0) or 0)
    if bscop_ref:
        m += f"🇻🇪➡️🇨🇴  *Referencia mercado BS→COP*\n      `{bscop_ref:.4f} COP/Bs`\n      _Automática — referencia aproximada de mercado_\n\n"
    else:
        m += "🇻🇪➡️🇨🇴  *Referencia mercado BS→COP*\n      _No disponible; requiere límites de mercado actualizados_\n\n"
    ...
    # CLP→COP: referencia GSA rápida, escenario base limpio (sin referido,
    # sin delivery y sin corresponsal) con margen recomendado del 7%.
    try:
        ref_bs_cop = float(d.get('limite_bs_cop', 0) or 0)
        ref_clp_bs = float(d.get('limite_clp_bs', 0) or 0)
        if ref_bs_cop > 0 and ref_clp_bs > 0:
            capacidad_cop_por_clp = (ref_clp_bs * ref_bs_cop) / (1 + COMISION_COMPRA_COP)
            tasa_ref_gsa_7 = capacidad_cop_por_clp / (1 + MARGEN_RECOMENDADO)
            m += f"🇨🇱➡️🇨🇴  CLP → COP     `Ref. GSA 7%: {tasa_ref_gsa_7:.4f}`\n"
        else:
            m += "🇨🇱➡️🇨🇴  CLP → COP     `Ref. GSA 7%: N/D`\n"
    except Exception:
        m += "🇨🇱➡️🇨🇴  CLP → COP     `Ref. GSA 7%: N/D`\n"
    ...
    if bscop_ref:
        m += f"🇻🇪➡️🇨🇴  Bs → COP      `Ref. mercado {bscop_ref:.4f}`\n"
    m += "\n"
    ...
    # Límites referenciales (se conservan como referencia de mercado).
    # IMPORTANTE: Western puede formar estos límites aproximados legacy,
    # pero NO forma la tasa comercial CLP→COP del Motor Pricing V1.
    m += f"━━━━━━━━━━━━━━━━━━━━\n📐 *LÍMITES REFERENCIALES*\n\n"
    if d.get('limite_clp_bs'):
        m += f"🔴  *Límite CLP/Bs*\n      `{fmt(d['limite_clp_bs'],6)}`\n\n"
    if d.get('limite_clp_cop'):
        m += f"🔴  *Límite CLP/COP*\n      `{fmt(d['limite_clp_cop'],4)}`\n\n"
    if d.get('limite_bs_cop'):
        m += f"🔴  *Límite BS/COP*\n      `{fmt(d['limite_bs_cop'],4)}`\n\n"
    if not d.get('western'):
        m += "_CLP/COP y BS/COP requieren una referencia Western guardada con /western TASA._\n\n"
    else:
        m += "_Estos límites son referencias de mercado; no determinan el Pricing CLP→COP V1._\n\n"
...
ultimo_offset=0
try:
    _western_guardado = float(get_config('western_rate', '0') or 0)
    western_rate = _western_guardado if _western_guardado > 0 else None
except Exception:
    western_rate = None
ultimo_datos={}
...
# ══════════════════════════════════════════════════════════════════════
# GSA CAMBIOS V1 — ALCANCE ACTIVO
# Bot = tasas + mercado + pricing + análisis P2P.
# Excel = operaciones + inventario + saldos + contabilidad + CxC/CxP.
# El código legado se conserva por seguridad, pero queda fuera del flujo activo.
# ══════════════════════════════════════════════════════════════════════
MODO_V1_SIMPLE = True

COMANDOS_V1 = {
    '/tasas', '/western', '/limites',
    '/mercado', '/patron',
    '/simular', '/sim',
    '/profundidad', '/mianuncio',
    '/start', '/help', '/ayuda'
}
...
def procesar(chat_id, texto):
    ...
    # Los flujos ERP/ledger/importación quedan inactivos en V1 simple.
    if not MODO_V1_SIMPLE:
        if hay_conv_activa(chat_id) and not texto.startswith('/'):
            send(chat_id, procesar_conv(chat_id, texto)); return

        # Detectar formulario de operación rápida (legado)
    ...
    # En V1 solo están activos los comandos del asistente de tasas/pricing.
    if MODO_V1_SIMPLE and cmd.startswith('/') and cmd not in COMANDOS_V1:
        send(chat_id,
             "ℹ️ *Este comando no está activo en GSA Cambios V1.*\n\n"
             "El Bot está enfocado en:\n"
             "• Tasas\n"
             "• Mercado\n"
             "• Cotizaciones\n"
             "• Binance P2P\n\n"
    ...
    if cmd=='/tasas':
        ...
        send(chat_id, construir_mensaje_tasas_v1(datos))

    elif cmd=='/western':
        # V1: Western es benchmark y servicio con comisión GSA de 5% sobre nominal.
        # Compatibilidad histórica: Comisión GSA (5%). Western sigue siendo benchmark. El 5% se descuenta del nominal.
        # No se aplica automáticamente el fee legacy de 2,5%.
        if len(partes)>=2:
            try:
                ...
                set_config('western_rate', str(western_rate))
                set_config('western_actualizado_hoy', str(today_local()))

                nominal_ejemplo = 500000.0
                r_w = cotizar_western_cop(
                    nominal_ejemplo,
                    costos_ejecucion=0.0
                )

                send(chat_id,
                     f"✅ *WESTERN ACTUALIZADO*\\n\\n"
                     f"Referencia:\\n`{western_rate:.4f} COP/CLP`\\n\\n"
                     f"🌎 *WESTERN / CORRESPONSAL*\\n\\n"
                     f"Comisión GSA: *5%*\\n"
                     f"Calculada sobre el monto recibido por el corresponsal.\\n\\n"
                     f"Ejemplo:\\n"
                     f"Monto recibido: `{nominal_ejemplo:,.0f} COP`\\n"
                     f"Comisión GSA: `{r_w['comision_gsa']:,.0f} COP`\\n"
                     f"Monto neto: `{r_w['monto_cliente']:,.0f} COP`\\n\\n"
                     f"✅ Referencia guardada correctamente.")
            except Exception as e:
                send(chat_id, f"❌ Uso: `/western 3.3628`\nDetalle: {e}")
        else:
            send(chat_id,"Uso: `/western 3.3628`")


    elif cmd=='/bscop':
        # Ya no se guarda una tasa BS→COP global porque cada operación puede negociarse
        # con una tasa diferente. La referencia de mercado es automática.
        t_ref = ultimo_datos or get_ultima_tasa() or {}
        tasa_ref = float(t_ref.get('limite_bs_cop', 0) or 0)
        if tasa_ref > 0:
            m = f"📌 *BS→COP — REFERENCIA DE MERCADO*\n"
            m += f"`{tasa_ref:.4f} COP/Bs`\n\n"
            m += "Esta referencia es automática y aproximada.\n"
            m += "Para una operación concreta, coloca la tasa realmente negociada en:\n"
            m += "`TASA-BS-COP: X.XXXX` dentro de `/simular`.\n\n"
            m += "Si dejas `TASA-BS-COP: 0`, el Motor Pricing usará esta referencia y lo marcará como simulación referencial."
            send(chat_id, m)
        else:
            send(chat_id, "⚠️ No hay referencia BS→COP disponible. Ejecuta /tasas y verifica que Western esté registrado.")
        ...
        # V1 práctico: mesa rápida de negociación para operación estándar CLP→COP.
        # Sin referido, delivery, corresponsal ni otros gastos.
        # Las demás rutas se incorporarán cuando esté listo el motor universal.
        send(chat_id, msg_limites_clp_cop_v1())
    ...
    # ── SIMULAR — MOTOR UNIVERSAL V1 ──
        ...
        if len(lineas) <= 1:
            ...
            try:
                ...
                tipo_op = _normalizar_tipo_motor(campos.get('TIPO',''))
                monto = _float_monto(campos.get('MONTO','0'))

                if tipo_op not in TIPOS_MOTOR_V1:
                    send(chat_id, f"❌ Tipo no válido: `{tipo_op}`\nUsa /simular para ver las 18 operaciones.")
                else:
                    resultado = calcular_simulacion_motor_v1(tipo_op, monto, campos)
                    send(chat_id, msg_resultado_motor_v1(resultado))
                    msg_cliente = msg_cliente_motor_v1(resultado)
                    if msg_cliente:
                        send(chat_id, "📤 *MENSAJE PARA EL CLIENTE:*\n" + msg_cliente)
                    elif resultado.get('estado') == ESTADO_BLOQUEADO:
                        send(chat_id, "🔴 *Operación bloqueada.* No se generó mensaje para el cliente.")
            except Exception as e:
        ...
        if len(partes) >= 3:
            try:
                ...
                cliente = partes[3] if len(partes) >= 4 else 'Cliente'
                corresp = partes[4].replace('_',' ') if len(partes) >= 5 else ''
                t = get_ultima_tasa()
                resultado = calcular_simulacion(tipo_op, monto, cliente, corresp, t)
                send(chat_id, msg_resultado_simulacion(resultado))
            except Exception as e:
                send(chat_id, f"❌ Error: {e}\n\nUso: `/sim CLP→BS 500000 Juan Bancolombia_C1`")
        else:
            send(chat_id, msg_simular_menu())
    ...
    # ── SISTEMA ──
        ...
        send(chat_id,"""🤖 *GSA CAMBIOS — ASISTENTE DE TASAS*

*📊 TASAS Y SERVICIOS*
/tasas — Ver tasas comerciales y servicios disponibles
/western TASA — Actualizar referencia Western
/limites — Consultar tramos CLP → COP

*📡 MERCADO*
/mercado — Ver mercado y referencias en vivo
/patron — Analizar comportamiento histórico
  `/patron bs`
  `/patron hoy`
  `/patron semana`

*🧮 COTIZADOR*
/simular — Formato rápido para cotizar una operación

*📊 BINANCE P2P*
/mianuncio — Revisar posición del anuncio GSA

_GSA Cambios analiza tasas, mercado y rentabilidad. Las operaciones y registros administrativos se gestionan por separado._""")

    elif not MODO_V1_SIMPLE and hay_conv_activa(chat_id):
...
# ══════════════════════════════════════════════════════════════════════
# LECTOR DE SALDOS AUXILIARES
# ══════════════════════════════════════════════════════════════════════
def leer_saldos_auxiliares(ruta_archivo, fecha_hoja):
    """
    Lee los saldos de cierre del día desde los auxiliares bancarios.
    
    BANESCO/MERCANTIL: Última fila donde Col B = fecha → Col F = saldo
    COPEC: Última fila donde Col B = fecha → Col F = saldo
    AIRTM: Fila de totales generales → Col O = USDC neto
    BINANCE: Suma compras - ventas donde fecha = fecha_hoja
    
    Retorna dict con saldos de cierre por cuenta.
    """
    ...
    def es_fecha(v, fecha_obj):
        """Verifica si el valor de celda corresponde a la fecha buscada."""
        if v is None: return False
        if isinstance(v, datetime):
            return v.date() == fecha_obj
        if isinstance(v, date):
            return v == fecha_obj
        try:
            return datetime.strptime(str(v)[:10], '%Y-%m-%d').date() == fecha_obj
        except:
            return False

    saldos = {
        'BS_BANESCO':    {'saldo': None, 'filas': 0, 'error': None},
        'BS_MERCANTIL':  {'saldo': None, 'filas': 0, 'error': None},
        'CLP_COPEC_PAY': {'saldo': None, 'filas': 0, 'error': None},
        'USDT_BINANCE':  {'saldo': None, 'filas': 0, 'error': None},
        'USDC_AIRTM':    {'saldo': None, 'filas': 0, 'error': None},
    }
    ...
    # Parsear fecha
    try:
        if isinstance(fecha_hoja, str):
            fecha_obj = datetime.strptime(fecha_hoja, '%Y-%m-%d').date()
        else:
            fecha_obj = fecha_hoja
    except:
        for k in saldos:
    ...
    # ── MERCANTIL ──
    hojas_merc = ['Banco Mercantil', 'BANCO MERCANTIL', 'Mercantil', 'mercantil']
    ...
    # ── COPEC ──
    hojas_copec = ['Copec', 'COPEC', 'Copec Pay', 'copec']
...
def formatear_resultado_relacion_diaria_partes(resultado, guardado):
    ...
    # Primero construir set de clientes ya resueltos en hojas del día
    # (Cobrado total, Pagado, Cancelada) para excluirlos del mes anterior
    cxc_resueltas = set()
    for c in resultado.get('cxc_dia', []):
        if c.get('status','') not in ('Pendiente', 'Parcial', ''):
            cxc_resueltas.add((c.get('cliente',''), c.get('monto',0), c.get('moneda','')))

    cxp_resueltas = set()
    for c in resultado.get('cxp_dia', []):
        if c.get('status','') not in ('Pendiente', 'Parcial', ''):
            cxp_resueltas.add((c.get('acreedor',''), c.get('monto',0), c.get('moneda','')))

    m2 = ""
    cxc_vistas = set()
    cxc_pend = []
    # Primero las del día (estado más actualizado), luego mes anterior
    for c in resultado.get('cxc_dia', []) + resultado.get('cxc_mes_anterior', []):
        key = (c.get('cliente',''), c.get('monto',0), c.get('moneda',''))
        if key in cxc_resueltas: continue  # ya cobrada en hojas del día
        if key in cxc_vistas: continue     # ya procesada
        cxc_vistas.add(key)
    ...
    for c in resultado.get('cxp_dia', []) + resultado.get('cxp_mes_anterior', []):
        ...
        if key in cxp_resueltas: continue  # ya pagada en hojas del día
        if key in cxp_vistas: continue
        cxp_vistas.add(key)
        if c.get('status') == 'Pendiente':
            cxp_pend.append(c)
    if cxp_pend:
...
def formatear_resultado_relacion_diaria(resultado, guardado):
    ...
    # CXC pendientes — excluir las ya cobradas en hojas del día
    cxc_resueltas2 = set()
    for c in resultado.get('cxc_dia', []):
        if c.get('status','') not in ('Pendiente', 'Parcial', ''):
            cxc_resueltas2.add((c.get('cliente',''), c.get('monto',0), c.get('moneda','')))

    cxc_vistas = set()
    cxc_pend = []
    for c in resultado.get('cxc_dia', []) + resultado.get('cxc_mes_anterior', []):
        key = (c.get('cliente',''), c.get('monto',0), c.get('moneda',''))
        if key in cxc_resueltas2: continue
        if key in cxc_vistas: continue
        cxc_vistas.add(key)
        if c.get('status') == 'Pendiente':
            cxc_pend.append(c)
    if cxc_pend:
    ...
    # CXP pendientes — excluir las ya pagadas en hojas del día
    cxp_resueltas2 = set()
    for c in resultado.get('cxp_dia', []):
        if c.get('status','') not in ('Pendiente', 'Parcial', ''):
            cxp_resueltas2.add((c.get('acreedor',''), c.get('monto',0), c.get('moneda','')))

    cxp_vistas = set()
    cxp_pend = []
    for c in resultado.get('cxp_dia', []) + resultado.get('cxp_mes_anterior', []):
        key = (c.get('acreedor',''), c.get('monto',0), c.get('moneda',''))
        if key in cxp_resueltas2: continue
        if key in cxp_vistas: continue
        cxp_vistas.add(key)
        if c.get('status') == 'Pendiente':
            cxp_pend.append(c)
    ...
    # Saldos de cierre desde auxiliares
    saldos_cierre = resultado.get('saldos_cierre')
    if saldos_cierre:
        saldo_usdt_ini = resultado.get('saldo_inicial_usdt', 0)
        m += formatear_saldos_cierre(saldos_cierre, saldo_usdt_ini)
...
# ══════════════════════════════════════════════════════════════════════
# SIMULADOR DE COTIZACIÓN — /simular y /sim
# ══════════════════════════════════════════════════════════════════════
...
def calcular_simulacion(tipo_op, monto, cliente, corresponsal, t):
    """Calcula la cotización completa para una operación."""
    pat_bs  = ((t.get('ban_bs_compra',0) or 0)+(t.get('ban_bs_venta',0) or 0))/2 or 1
    dol_obs = t.get('dolar_obs', 1) or 1
    trm     = t.get('trm', 1) or 1
    western = t.get('western', 1) or 1

    # Límites base
    limite_clp_bs  = t.get('limite_clp_bs', 0) or 0
    limite_clp_cop = t.get('limite_clp_cop', 0) or 0
    limite_bs_cop  = t.get('limite_bs_cop', 0) or 0

    # Tasas GSA
    tasa_gsa_clp_bs  = t.get('tasa_gsa_clp_bs', 0) or 0
    tasa_gsa_bs_clp  = t.get('tasa_gsa_bs_clp', 0) or 0
    tasa_gsa_clp_cop = t.get('tasa_gsa_clp_cop', 0) or 0
    tasa_gsa_cop_clp = t.get('tasa_gsa_cop_clp', 0) or 0
    ...
    try:
        if tipo_op == 'CLP→BS':
            if not limite_clp_bs: res['error'] = 'Sin datos de mercado BS'; res['ok'] = False; return res
            res['mon_entrada'] = 'CLP'
            res['mon_salida'] = 'BS'
            res['tasa_limite'] = limite_clp_bs
            res['tasa_cliente'] = tasa_gsa_clp_bs
            res['tasa_referencia'] = limite_clp_bs
            res['monto_salida'] = round(monto * tasa_gsa_clp_bs, 2)
            res['usdt_equiv'] = round(monto / dol_obs, 4)
            res['gan_comercial'] = round(res['usdt_equiv'] * (limite_clp_bs - tasa_gsa_clp_bs) / limite_clp_bs, 4)

        elif tipo_op == 'BS→CLP':
            if not limite_clp_bs: res['error'] = 'Sin datos de mercado BS'; res['ok'] = False; return res
            res['mon_entrada'] = 'BS'
            res['mon_salida'] = 'CLP'
            res['tasa_limite'] = 1/limite_clp_bs if limite_clp_bs else 0
            res['tasa_cliente'] = tasa_gsa_bs_clp
            res['tasa_referencia'] = 1/limite_clp_bs if limite_clp_bs else 0
            res['monto_salida'] = round(monto * tasa_gsa_bs_clp, 2)
            res['usdt_equiv'] = round(monto / pat_bs, 4)
            res['gan_comercial'] = round(res['usdt_equiv'] * abs(tasa_gsa_bs_clp - (1/limite_clp_bs)) / (1/limite_clp_bs), 4) if limite_clp_bs else 0

        elif tipo_op == 'CLP→COP':
            if not limite_clp_cop: res['error'] = 'Sin Western Union'; res['ok'] = False; return res
            res['mon_entrada'] = 'CLP'
            res['mon_salida'] = 'COP'
            res['tasa_limite'] = limite_clp_cop
            res['tasa_cliente'] = tasa_gsa_clp_cop
            res['tasa_referencia'] = limite_clp_cop
            res['monto_salida'] = round(monto * tasa_gsa_clp_cop, 2)
            res['usdt_equiv'] = round(monto / dol_obs, 4)
            res['gan_comercial'] = round(res['usdt_equiv'] * (limite_clp_cop - tasa_gsa_clp_cop) / limite_clp_cop, 4)

        elif tipo_op == 'COP→CLP':
            if not limite_clp_cop: res['error'] = 'Sin Western Union'; res['ok'] = False; return res
            res['mon_entrada'] = 'COP'
            res['mon_salida'] = 'CLP'
            res['tasa_limite'] = 1/limite_clp_cop if limite_clp_cop else 0
            res['tasa_cliente'] = tasa_gsa_cop_clp
            res['tasa_referencia'] = 1/limite_clp_cop if limite_clp_cop else 0
            res['monto_salida'] = round(monto * tasa_gsa_cop_clp, 2)
            res['usdt_equiv'] = round(monto / trm, 4)
            res['gan_comercial'] = round(res['usdt_equiv'] * 0.075, 4)

        elif tipo_op == 'BS→COP':
            if not limite_bs_cop: res['error'] = 'Sin datos BS/COP'; res['ok'] = False; return res
            res['mon_entrada'] = 'BS'
            res['mon_salida'] = 'COP'
            res['tasa_limite'] = limite_bs_cop
            res['tasa_cliente'] = tasa_gsa_bs_cop
            res['tasa_referencia'] = limite_bs_cop
            res['monto_salida'] = round(monto * tasa_gsa_bs_cop, 2)
            res['usdt_equiv'] = round(monto / pat_bs, 4)
            ...
            res['mon_entrada'] = 'CLP'
            res['mon_salida'] = 'USDT'
            res['tasa_cliente'] = precio_clp
            res['tasa_limite'] = precio_clp
            res['tasa_referencia'] = precio_clp
            res['monto_salida'] = round(monto / precio_clp, 4) if precio_clp else 0
            res['usdt_equiv'] = res['monto_salida']
            res['gan_comercial'] = 0
            res['notas'] = 'Operación de liquidez — ganancia en el spread de Binance'

        elif tipo_op == 'USDT→CLP':
            precio_clp = t.get('clp_venta', 0) or dol_obs
            res['mon_entrada'] = 'USDT'
            res['mon_salida'] = 'CLP'
            res['tasa_cliente'] = precio_clp
            res['tasa_limite'] = precio_clp
            res['tasa_referencia'] = precio_clp
            res['monto_salida'] = round(monto * precio_clp, 2)
            res['usdt_equiv'] = monto
            res['gan_comercial'] = 0
            res['notas'] = 'Operación de liquidez — ganancia en el spread de Binance'
            ...
            res['mon_entrada'] = 'USDT'
            res['mon_salida'] = 'BS'
            precio_bs_v = t.get('ban_bs_venta', 0) or pat_bs
            res['tasa_cliente'] = precio_bs_v
            res['tasa_limite'] = precio_bs_v
            res['tasa_referencia'] = precio_bs_v
            res['monto_salida'] = round(monto * precio_bs_v, 2)
            res['usdt_equiv'] = monto
            res['gan_comercial'] = 0
            res['notas'] = 'Venta USDT en Binance — ganancia = precio venta - CPP'

        elif tipo_op == 'BS→USDT':
            res['mon_entrada'] = 'BS'
            res['mon_salida'] = 'USDT'
            precio_bs_c = t.get('ban_bs_compra', 0) or pat_bs
            res['tasa_cliente'] = precio_bs_c
            res['tasa_limite'] = precio_bs_c
            res['tasa_referencia'] = precio_bs_c
            res['monto_salida'] = round(monto / precio_bs_c, 4) if precio_bs_c else 0
            res['usdt_equiv'] = res['monto_salida']
            res['gan_comercial'] = 0
            res['notas'] = 'Compra USDT en Binance — actualiza inventario CPP'

        elif tipo_op == 'CLP→USD':
            res['mon_entrada'] = 'CLP'
            res['mon_salida'] = 'USD'
            tasa = dol_obs * 1.055
            res['tasa_cliente'] = round(tasa, 2)
            res['tasa_limite'] = dol_obs
            res['tasa_referencia'] = dol_obs
            res['monto_salida'] = round(monto / tasa, 4)
            res['usdt_equiv'] = round(monto / dol_obs, 4)
            res['gan_comercial'] = round(res['usdt_equiv'] * 0.055, 4)
            ...
            res['tasa_limite'] = dol_obs
            res['tasa_referencia'] = dol_obs
            res['monto_salida'] = round(monto * tasa, 2)
            res['usdt_equiv'] = monto
            res['gan_comercial'] = round(monto * 0.055, 4)

        elif tipo_op == 'BS→USD':
            tasa_bs_usd = pat_bs - 30
            res['mon_entrada'] = 'BS'
            res['mon_salida'] = 'USD'
            res['tasa_cliente'] = tasa_bs_usd
            res['tasa_limite'] = pat_bs
            res['tasa_referencia'] = pat_bs
            res['monto_salida'] = round(monto / tasa_bs_usd, 4) if tasa_bs_usd else 0
            res['usdt_equiv'] = round(monto / pat_bs, 4)
            res['gan_comercial'] = round(res['usdt_equiv'] * 0.04, 4)

        elif tipo_op == 'USD→BS':
            tasa_usd_bs = pat_bs + 30
            res['mon_entrada'] = 'USD'
            res['mon_salida'] = 'BS'
            res['tasa_cliente'] = tasa_usd_bs
            res['tasa_limite'] = pat_bs
            res['tasa_referencia'] = pat_bs
            res['monto_salida'] = round(monto * tasa_usd_bs, 2)
            res['usdt_equiv'] = monto
            res['gan_comercial'] = round(monto * 0.04, 4)
            ...
            res['mon_entrada'] = 'COP'
            res['mon_salida'] = 'USDT'
            res['tasa_cliente'] = trm
            res['tasa_limite'] = trm
            res['tasa_referencia'] = trm
            res['monto_salida'] = round(monto / trm, 4)
            res['usdt_equiv'] = res['monto_salida']
            res['gan_comercial'] = 0
            res['notas'] = 'Operación de liquidez COP'
            ...
            res['tasa_limite'] = trm
            res['tasa_referencia'] = trm
            res['monto_salida'] = round(monto * trm, 2)
            res['usdt_equiv'] = monto
            res['gan_comercial'] = 0
            res['notas'] = 'Operación de liquidez COP'

        elif tipo_op == 'BS→USDC':
            precio = t.get('ban_bs_compra', 0) or pat_bs
            res['mon_entrada'] = 'BS'
            res['mon_salida'] = 'USDC'
            res['tasa_cliente'] = precio
            res['tasa_limite'] = precio
            res['tasa_referencia'] = precio
            res['monto_salida'] = round(monto / precio, 4) if precio else 0
            res['usdt_equiv'] = res['monto_salida']
            res['gan_comercial'] = 0
            res['notas'] = 'Compra USDC en Airtm'

        elif tipo_op == 'USDC→BS':
            precio = t.get('ban_bs_venta', 0) or pat_bs
            res['mon_entrada'] = 'USDC'
            res['mon_salida'] = 'BS'
            res['tasa_cliente'] = precio
            res['tasa_limite'] = precio
            res['tasa_referencia'] = precio
            res['monto_salida'] = round(monto * precio, 2)
            res['usdt_equiv'] = monto
            res['gan_comercial'] = 0
            res['notas'] = 'Venta USDC en Airtm'
            ...
            res['tasa_cliente'] = 1
            res['tasa_limite'] = 1
            res['tasa_referencia'] = 1
            com_airtm = round(monto * 0.007, 4)
            res['monto_salida'] = round(monto - com_airtm, 4)
            res['usdt_equiv'] = res['monto_salida']
            res['gan_comercial'] = 0
            res['notas'] = f'Retiro Airtm→Binance. Comisión Airtm ~0.7%: {com_airtm} USDC'

        else:
            res['error'] = f'Tipo de operación no reconocido: {tipo_op}'
            res['ok'] = False
            return res

        # Comisión corresponsal 2.5%
        if corresponsal and corresponsal.lower() not in ('', 'ninguno', 'no'):
            res['com_corresponsal'] = round(res['usdt_equiv'] * 0.025, 4)
        else:
            res['com_corresponsal'] = 0

        res['gan_neta'] = round(res['gan_comercial'] - res['com_corresponsal'], 4)
...
def seleccionar_margen_clp_cop(monto_clp, margen_solicitado='AUTO'):
    """Política comercial V1 CLP→COP.
    ...
    Manual:
      - 7, 5 o 3
    La utilidad mínima absoluta se protege aparte en el cálculo de la tasa.
    """
    monto = float(monto_clp or 0)
    raw = str(margen_solicitado or 'AUTO').strip().upper().replace('%', '')

    if raw in ('', 'AUTO'):
        if monto <= 100000:
            return {
                'modo': 'AUTO',
                'margen': MARGEN_RECOMENDADO,
                'margen_pct': 7.0,
                'etiqueta': '7% RECOMENDADA',
                'politica': 'Hasta 100.000 CLP → 7%',
            }
        return {
            'modo': 'AUTO',
            'margen': MARGEN_PREFERENCIAL,
            'margen_pct': 5.0,
            'etiqueta': '5% PREFERENCIAL',
            'politica': 'Desde 100.001 CLP → 5%',
        }

    mapa = {
        '7': (MARGEN_RECOMENDADO, '7% RECOMENDADA'),
        '7.0': (MARGEN_RECOMENDADO, '7% RECOMENDADA'),
        '5': (MARGEN_PREFERENCIAL, '5% PREFERENCIAL'),
        '5.0': (MARGEN_PREFERENCIAL, '5% PREFERENCIAL'),
        '3': (MARGEN_CAPTACION, '3% CAPTACIÓN'),
        '3.0': (MARGEN_CAPTACION, '3% CAPTACIÓN'),
    }
    if raw not in mapa:
        raise ValueError("MARGEN debe ser AUTO, 7, 5 o 3.")

    margen, etiqueta = mapa[raw]
    return {
        'modo': 'MANUAL',
        'margen': margen,
        'margen_pct': margen * 100.0,
        'etiqueta': etiqueta,
        'politica': f'Selección manual → {etiqueta}',
    }


def calcular_tasa_objetivo_con_piso(
    monto_clp, cop_potenciales, margen_objetivo,
    referido=False, delivery_cop=0.0, avance_corresponsal_cop=0.0,
    utilidad_minima_cop=UTILIDAD_MINIMA_COP
):
    """Calcula la tasa máxima al cliente protegiendo:
    1) el margen porcentual elegido, y
    2) la utilidad mínima absoluta.
    Se usa el criterio más conservador.
    """
    monto = float(monto_clp or 0)
    capacidad = float(cop_potenciales or 0)
    if monto <= 0 or capacidad <= 0:
        raise ValueError("Monto y capacidad deben ser mayores a cero.")

    r = COMISION_REFERIDO if referido else 0.0
    costo_corr = float(avance_corresponsal_cop or 0) * COMISION_CORRESPONSAL
    costos_fijos = float(delivery_cop or 0) + costo_corr

    # Por margen porcentual: C = P*(1+r+u) + costos_fijos
    p_porcentaje = (capacidad - costos_fijos) / (1.0 + r + float(margen_objetivo))

    # Por utilidad absoluta: U = C - P - rP - costos_fijos >= utilidad_minima
    p_utilidad = (capacidad - costos_fijos - float(utilidad_minima_cop)) / (1.0 + r)

    p_cliente = max(0.0, min(p_porcentaje, p_utilidad))
    tasa = p_cliente / monto if monto else 0.0
    utilidad = capacidad - p_cliente - (r * p_cliente) - costos_fijos
    rentabilidad = utilidad / p_cliente if p_cliente > 0 else 0.0

    return {
        'tasa': round(tasa, 6),
        'cop_cliente': round(p_cliente, 2),
        'utilidad_cop': round(utilidad, 2),
        'rentabilidad': rentabilidad,
        'rentabilidad_pct': round(rentabilidad * 100.0, 4),
        'piso_activo': p_utilidad < p_porcentaje,
    }



def msg_limites_clp_cop_v1():
    """Tabla rápida de tramos CLP→COP vigente."""
    t = get_ultima_tasa() or {}
    info = obtener_tramos_clp_cop(t, persistir=True)
    if not info["tramos"]:
        return "⚠️ No hay referencia suficiente para calcular CLP→COP. Ejecuta /tasas primero."

    t1,t2,t3,t4 = [x[1] for x in info["tramos"]]
    ahora = now_local().strftime("%d/%m/%Y — %I:%M %p")

    m = "📐 *GSA CAMBIOS — LÍMITES CLP → COP*\n"
    m += "━━━━━━━━━━━━━━━━━━━━\n\n"
    m += "Tasas vigentes:\n\n"
    m += f"• Hasta 25.000 CLP\n  → `{t1:.4f} COP/CLP`\n\n"
    m += f"• 25.001 – 50.000 CLP\n  → `{t2:.4f} COP/CLP`\n\n"
    m += f"• 50.001 – 150.000 CLP\n  → `{t3:.4f} COP/CLP`\n\n"
    m += f"• Desde 150.001 CLP\n  → `{t4:.4f} COP/CLP`\n\n"
    m += "📌 Para operaciones con referido, delivery, corresponsal u otros costos utiliza /simular.\n\n"
    m += f"Última consulta: `{ahora}`"
    return m

# ══════════════════════════════════════════════════════════════════════
# MOTOR UNIVERSAL V1 — ADAPTADOR BOT
# ══════════════════════════════════════════════════════════════════════
TIPOS_MOTOR_V1 = [
    "CLP→BS", "BS→CLP", "CLP→COP", "COP→CLP",
    "CLP→USD", "USD→CLP", "CLP→USDT", "USDT→CLP",
    "WESTERN→COP", "COP→BS", "BS→COP", "USD→BS",
    "BS→USD", "USD→COP", "COP→USD", "USDT→COP",
    "USDT→USD", "USD→USDT",
]

def _campo_bool(campos, nombre, default=False):
    v = str(campos.get(nombre, "SI" if default else "NO")).strip().upper()
    return v in ("SI", "SÍ", "YES", "TRUE", "1", "ACTIVO", "ALTA")

def _campo_num(campos, nombre, default=0.0):
    v = campos.get(nombre)
    if v is None or str(v).strip() == "":
        return float(default)
    s = str(v).strip().replace(" ", "")
    # admite decimal con coma; para miles usar MONTO y _float_monto.
    if "," in s and "." not in s:
        s = s.replace(",", ".")
    return float(s)

def _normalizar_tipo_motor(tipo_raw):
    s = str(tipo_raw or "").upper().strip()
    s = s.replace(" ", "").replace("-", "→").replace(">", "→")
    alias = {
        "WESTERNCOP": "WESTERN→COP",
        "WESTERN→COP": "WESTERN→COP",
        "GIROWESTERN": "WESTERN→COP",
        "GIROINT": "WESTERN→COP",
    }
    if s.replace("→", "") in alias:
        return alias[s.replace("→", "")]
    for tipo in TIPOS_MOTOR_V1:
        if s.replace("→", "") == tipo.replace("→", ""):
            return tipo
    return s

def _estado_visual_v1(estado):
    return {
        ESTADO_RENTABLE: "🟢 RENTABLE",
        ESTADO_MARGEN_COMPRIMIDO: "🟡 MARGEN COMPRIMIDO",
        ESTADO_CAPTACION: "🟠 CAPTACIÓN",
        ESTADO_ESTRATEGICA: "🟠 ESTRATÉGICA",
        ESTADO_NO_RECOMENDADO: "🟡 NO RECOMENDADO",
        ESTADO_BLOQUEADO: "🔴 BLOQUEAR",
    }.get(estado, str(estado))

def _fmt_moneda_v1(valor, moneda):
    if valor is None:
        return "-"
    if moneda in ("CLP", "COP"):
    ...
    if moneda == "USDT":
        return f"{valor:,.4f} USDT"
    return f"{valor:,.2f} {moneda}"

def _capacidad_clp_bs_actual(t, monto_clp):
    tasa_clp_usdt = float(t.get("clp_compra", 0) or t.get("dolar_obs", 0) or 0)
    tasa_usdt_bs = max(float(t.get("ban_bs_venta", 0) or 0), float(t.get("mer_bs_venta", 0) or 0))
    if tasa_clp_usdt <= 0 or tasa_usdt_bs <= 0:
        raise ValueError("Sin mercado CLP→USDT→BS")
    ciclo = producir_bs_desde_clp(
        monto_clp=float(monto_clp),
        tasa_clp_usdt=tasa_clp_usdt,
        tasa_usdt_bs=tasa_usdt_bs,
        fee_clp_usdt=PRICING_FEE_CLP_USDT,
        fee_usdt_bs=PRICING_FEE_USDT_BS,
    )
    return float(ciclo["bs_netos"])

def calcular_simulacion_motor_v1(tipo_op, monto, campos=None, tasas=None):
    """Adaptador entre Telegram y pricing_gsa V1.

    Solo solicita por campos manuales aquello que no existe en las fuentes
    automáticas del Bot (p. ej. mercado físico Cúcuta o costo de reposición).
    """
    campos = campos or {}
    t = tasas or get_ultima_tasa() or {}
    tipo = _normalizar_tipo_motor(tipo_op)
    monto = float(monto)
    if monto <= 0:
        raise ValueError("MONTO debe ser mayor a cero")

    dol = float(t.get("dolar_obs", 0) or 0)
    western = float(t.get("western", 0) or 0)
    clp_buy = float(t.get("clp_compra", 0) or 0)
    clp_sell = float(t.get("clp_venta", 0) or 0)
    cop_buy = float(t.get("cop_compra", 0) or 0)
    cop_sell = float(t.get("cop_venta", 0) or 0)
    bs_buy = max(float(t.get("ban_bs_compra", 0) or 0), float(t.get("mer_bs_compra", 0) or 0))
    bs_sell = max(float(t.get("ban_bs_venta", 0) or 0), float(t.get("mer_bs_venta", 0) or 0))

    # Campos de ejecución opcionales.
    maker = _campo_num(campos, "PRECIO_MAKER", 0)
    taker = _campo_num(campos, "PRECIO_TAKER", 0)
    modo = str(campos.get("MODO", "AUTO")).upper()
    maker_ok = maker > 0
    taker_ok = taker > 0

    if tipo == "CLP→BS":
        capacidad = _capacidad_clp_bs_actual(t, monto)
        tasa_manual = _campo_num(campos, "TASA_CLIENTE", 0)
        return cotizar_clp_bs(
            monto, capacidad,
            tasa_cliente=tasa_manual if tasa_manual > 0 else None,
            margen=str(campos.get("MARGEN", "AUTO")),
            costos_pendientes=_campo_num(campos, "OTROS", 0),
        )

    if tipo == "BS→CLP":
        if bs_buy <= 0 or clp_sell <= 0:
            raise ValueError("Sin mercado suficiente BS→USDT→CLP")
        limite_clp_bs = float(t.get("limite_clp_bs", 0) or 0)
        if limite_clp_bs <= 0:
            raise ValueError("Sin límite CLP/BS actualizado")
        usdt = monto / bs_buy
        capacidad = usdt * clp_sell
        return cotizar_bs_clp(
            monto, capacidad,
            tasa_bs_por_clp=limite_clp_bs * 1.10
        )

    if tipo == "CLP→COP":
        q = _campo_num(campos, "TASA_BS_COP", 0) or get_bscop_referencia(t)
        if q <= 0:
            raise ValueError("Falta TASA-BS-COP o referencia automática")
        capacidad_bs = _capacidad_clp_bs_actual(t, monto)
        capacidad_cop = calcular_capacidad_cop(capacidad_bs, q)
        k = capacidad_cop / monto
        tasa_manual = _campo_num(campos, "TASA_CLIENTE", 0)
        tasa_publicada = tasa_clp_cop_por_monto(t, monto, persistir=True)
        tasa_elegida = tasa_manual if tasa_manual > 0 else tasa_publicada
        monto_cliente_estimado = monto * tasa_elegida
        costos_solicitud = _campo_num(campos, "DELIVERY", 0) + _campo_num(campos, "OTROS", 0)
        if _campo_bool(campos, "REFERIDO", False):
            costos_solicitud += monto_cliente_estimado * COMISION_REFERIDO
        if _campo_bool(campos, "CORRESPONSAL", False):
            costos_solicitud += monto_cliente_estimado * COMISION_CORRESPONSAL
        return cotizar_clp_cop(
            monto, k,
            permitir_captacion=True,
            costos_pendientes=costos_solicitud,
            tasa_cliente=tasa_elegida,
        )

    if tipo == "COP→CLP":
        if western <= 0:
            raise ValueError("Sin referencia Western")
        return cotizar_cop_clp(monto, western, 3.15)

    if tipo == "CLP→USD":
        if dol <= 0:
            raise ValueError("Sin Dólar Observado")
        cr = _campo_num(campos, "COSTO_REPOSICION", 0)
        return cotizar_clp_usd(monto, dol, costo_reposicion_usd_clp=cr if cr > 0 else None)

    if tipo == "USD→CLP":
        if dol <= 0:
            raise ValueError("Sin Dólar Observado")
        return cotizar_usd_clp(monto, dol)

    if tipo == "CLP→USDT":
        if not taker_ok and clp_buy > 0:
            taker, taker_ok = clp_buy, True
        return cotizar_clp_usdt(
            monto, precio_maker_neto=maker if maker_ok else None,
            precio_taker_neto=taker if taker_ok else None,
            maker_disponible=maker_ok, taker_disponible=taker_ok, modo=modo
        )

    if tipo == "USDT→CLP":
        if not taker_ok and clp_sell > 0:
            taker, taker_ok = clp_sell, True
        return cotizar_usdt_clp(
            monto, capacidad_maker=maker if maker_ok else None,
            capacidad_taker=taker if taker_ok else None,
            maker_disponible=maker_ok, taker_disponible=taker_ok, modo=modo
        )

    if tipo == "WESTERN→COP":
        return cotizar_western_cop(monto, costos_ejecucion=_campo_num(campos, "COSTOS_EJECUCION", 0))

    if tipo == "COP→BS":
        if cop_buy <= 0 or bs_sell <= 0:
            raise ValueError("Sin mercado suficiente COP→USDT→BS")
        capacidad = (monto / cop_buy) * bs_sell
        fee_bs = 0.003 if _campo_bool(campos, "BANESCO_OTRO_BANCO", False) else 0.0
        capacidad -= capacidad * fee_bs
        return cotizar_cop_bs(monto, capacidad)

    if tipo == "BS→COP":
        if bs_buy <= 0 or cop_sell <= 0:
            raise ValueError("Sin mercado suficiente BS→USDT→COP")
        capacidad_binance = (monto / bs_buy) * cop_sell
        q_directa = _campo_num(campos, "TASA_DIRECTA", 0)
        capacidad_directa = monto * q_directa if q_directa > 0 else None
        return cotizar_bs_cop(
            monto, capacidad_binance,
            capacidad_directa_cop=capacidad_directa,
            directa_disponible=q_directa > 0,
        )

    if tipo == "USD→BS":
        if bs_sell <= 0:
            raise ValueError("Sin referencia Binance BS")
        ve = _campo_num(campos, "VALOR_ECONOMICO", 0)
        if ve <= 0:
            raise ValueError("Falta VALOR-ECONOMICO del USD físico en BS")
        return cotizar_usd_bs(
            monto, bs_sell, valor_economico_usd_bs=ve,
            cuenta_origen="BANESCO" if _campo_bool(campos, "BANESCO", False) else "",
            mismo_banco=not _campo_bool(campos, "BANESCO_OTRO_BANCO", False),
        )

    if tipo == "BS→USD":
        tasa = _campo_num(campos, "TASA_CLIENTE", 0) or bs_sell
        cr = _campo_num(campos, "COSTO_REPOSICION", 0)
        if tasa <= 0 or cr <= 0:
            raise ValueError("Falta TASA-CLIENTE o COSTO-REPOSICION USD/BS")
        return cotizar_bs_usd(monto, tasa, costo_reposicion_bs_usd=cr)

    if tipo in ("USD→COP", "COP→USD"):
        mc = _campo_num(campos, "MERCADO_COMPRA", 0)
        mv = _campo_num(campos, "MERCADO_VENTA", 0)
        if mc <= 0 or mv <= 0:
            raise ValueError("Faltan MERCADO-COMPRA y MERCADO-VENTA del USD físico en Cúcuta")
        if tipo == "USD→COP":
            ve = _campo_num(campos, "VALOR_ECONOMICO", 0)
            return cotizar_usd_cop(
                monto, mc, mv,
                valor_economico_min_usd_cop=ve if ve > 0 else None
            )
        cr = _campo_num(campos, "COSTO_REPOSICION", 0)
        if cr <= 0:
            raise ValueError("Falta COSTO-REPOSICION del USD físico en COP")
        return cotizar_cop_usd(monto, mc, mv, costo_reposicion_usd_cop=cr)

    if tipo == "USDT→COP":
        if not taker_ok and cop_sell > 0:
            taker, taker_ok = cop_sell, True
        return cotizar_usdt_cop(
            monto, capacidad_maker=maker if maker_ok else None,
            capacidad_taker=taker if taker_ok else None,
            maker_disponible=maker_ok, taker_disponible=taker_ok, modo=modo
        )
    ...
    if tipo == "USDT→USD":
        costo = _campo_num(campos, "COSTO_REPOSICION", 0)
        if costo <= 0:
            raise ValueError("Falta COSTO-REPOSICION en USDT por USD físico")
        # En esta operación MONTO representa USD que el cliente desea recibir.
        return cotizar_usdt_usd(monto, costo_economico_usdt_por_usd=costo)

    if tipo == "USD→USDT":
        necesidad = str(campos.get("NECESIDAD_USD", "ALTA")).upper()
        costo = _campo_num(campos, "COSTO_USDT", 1.0)
        return cotizar_usd_usdt(monto, necesidad_usd=necesidad, costo_usdt=costo)

    raise ValueError(f"Tipo no soportado por Motor V1: {tipo}")



ALERTAS_TELEGRAM_V1 = {
    ALERTA_MARGEN_COMPRIMIDO:
        "Margen por debajo del objetivo normal. Revisar antes de confirmar.",
    ALERTA_CAPTACION:
        "Operación rentable bajo criterio de captación/rotación, no bajo margen normal.",
    ALERTA_POLITICA_PROVISIONAL:
        "La política comercial de esta ruta aún es provisional.",
    ALERTA_BANESCO_003:
        "Aplica 0,30% por salida Banesco hacia otro banco.",
    ALERTA_MAKER_NO_DISPONIBLE:
        "No hay precio maker disponible; se evaluó la alternativa ejecutable.",
    ALERTA_TAKER_NO_DISPONIBLE:
        "No hay precio taker disponible; se evaluó la alternativa ejecutable.",
    ALERTA_COSTO_REPOSICION:
        "El costo de reposición compromete la rentabilidad de la operación.",
    ALERTA_INVENTARIO:
        "La operación depende de necesidad o disponibilidad de inventario.",
    ALERTA_WESTERN_BAJO_UMBRAL:
        "Western está bajo el piso económico del ciclo; se aplicó el umbral mínimo.",
    ALERTA_MERCADO_FISICO_INVALIDO:
        "Mercado físico inválido: revisar precios de compra y venta.",
    ALERTA_DATO_INSUFICIENTE:
        "Falta un dato necesario para autorizar la cotización.",
    ALERTA_LIQUIDEZ_INSUFICIENTE:
        "No existe liquidez/ejecución suficiente para completar la ruta.",
    ALERTA_OPERACION_NO_RENTABLE:
        "La operación no cubre su costo económico. No confirmar al cliente.",
    ALERTA_INPUT_INVALIDO:
        "Existe un dato de entrada inválido. Revisar la simulación.",
    ALERTA_SPREAD_PROVISIONAL_1:
        "USDT→COP utiliza temporalmente un spread comercial de 1%.",
}

def traducir_alerta_v1(codigo):
    """Convierte códigos internos del motor en texto operativo para Telegram."""
    return ALERTAS_TELEGRAM_V1.get(
        codigo,
        f"Alerta interna no catalogada: {codigo}"
    )

def es_resultado_autorizable_v1(r):
    """Determina si una cotización puede generar mensaje para cliente."""
    return (
        r.get("estado") != ESTADO_BLOQUEADO
        and r.get("monto_cliente") is not None
        and float(r.get("monto_cliente") or 0) >= 0
    )


def msg_resultado_motor_v1(r):
    """Vista interna del operador: incluye economía y alertas traducidas."""
    m = "🧮 *GSA — MOTOR V1*\n"
    m += "━━━━━━━━━━━━━━━━━━━━\n"
    m += f"Operación: `{r.get('operacion','-').replace('_',' → ')}`\n"
    m += f"Entrada: `{_fmt_moneda_v1(r.get('monto_origen'), r.get('moneda_origen',''))}`\n"
    if r.get("modo_ejecucion"):
        m += f"Ejecución: `{r['modo_ejecucion']}`\n"
    if r.get("ruta_economica"):
        m += f"Ruta económica: `{r['ruta_economica']}`\n"
    if r.get("tasa_comercial") is not None:
        m += f"Tasa comercial: `{r['tasa_comercial']:.6f}`\n"
    if r.get("monto_cliente") is not None:
        m += f"Cliente recibe: `{_fmt_moneda_v1(r['monto_cliente'], r.get('moneda_destino',''))}`\n"
    if r.get("capacidad_bruta") is not None:
        m += f"Capacidad bruta: `{r['capacidad_bruta']:,.2f}`\n"
    if r.get("costos_totales") is not None:
        m += f"Costos: `{r['costos_totales']:,.2f}`\n"
    if r.get("capacidad_neta") is not None:
        m += f"Capacidad neta: `{r['capacidad_neta']:,.2f}`\n"
    if r.get("utilidad_esperada") is not None:
        m += f"Utilidad esperada: `{r['utilidad_esperada']:,.2f}`\n"
    if r.get("margen_esperado") is not None:
        m += f"Margen: `{r['margen_esperado']*100:.2f}%`\n"
    m += f"Estado: *{_estado_visual_v1(r.get('estado'))}*\n"
    if r.get("policy_status") == POLICY_PROVISIONAL:
        m += "Política: `PROVISIONAL`\n"
    if r.get("alertas"):
        m += "\n⚠️ *Alertas operativas*\n"
        for codigo in r["alertas"]:
            m += f"• {traducir_alerta_v1(codigo)}\n"
    return m


def msg_cliente_motor_v1(r):
    """Vista limpia para copiar/pegar al cliente.

    Prohibido incluir ruta, capacidad, utilidad, margen, policy_status,
    modo de ejecución o códigos de alerta internos.
    """
    if not es_resultado_autorizable_v1(r):
        return None

    origen = r.get("moneda_origen", "")
    destino = r.get("moneda_destino", "")
    monto_destino = r.get("monto_cliente")
    tasa = r.get("tasa_comercial")

    m = "💱 *GSA Cambios*\n"
    m += "━━━━━━━━━━━━━━━━━━━━\n"
    m += f"Envías: `{_fmt_moneda_v1(r.get('monto_origen'), origen)}`\n"
    if tasa is not None:
        m += f"Tasa: `{tasa:.4f}`\n"
    m += f"Recibes: `{_fmt_moneda_v1(monto_destino, destino)}`\n\n"
    m += "✅ *Monto final estimado*\n"
    m += "_Cotización sujeta a confirmación al momento de realizar la operación._"
    return m



def msg_simular_ayuda():
    """Plantilla rápida aprobada para copiar/pegar desde el celular."""
    return (
        "🧮 *SIMULACIÓN RÁPIDA*\n\n"
        "Copia, completa y envía:\n\n"
        "`/simular\n"
        "TIPO:\n"
        "MONTO:\n"
        "REFERIDO: NO\n"
...
def _float_monto(valor, default=0.0):
    """Lee montos escritos como 300000, 300.000 o 300,000."""
    if valor is None or str(valor).strip() == '':
        return float(default)
    txt = str(valor).strip().replace(' ', '')
    # Para campos de monto tratamos puntos/comas como separadores de miles.
    txt = txt.replace('.', '').replace(',', '')
    return float(txt or default)


def get_bscop_referencia(datos=None):
    """Referencia automática BS→COP derivada de los límites de mercado.

    No representa una tasa negociada ni se guarda como tasa global.
    """
    try:
        t = datos or get_ultima_tasa() or {}
        v = float(t.get('limite_bs_cop', 0) or 0)
        if v > 0:
            return v

        # Fallback defensivo: reconstruir desde CLP/COP y CLP/BS si están disponibles.
        lcc = float(t.get('limite_clp_cop', 0) or 0)
        lcb = float(t.get('limite_clp_bs', 0) or 0)
        if lcc > 0 and lcb > 0:
            return lcc / lcb
    except Exception:
        pass
...
def calcular_cotizacion_clp_cop_v1(
    ...
    avance_corresponsal_cop=0.0, otros_cop=0.0,
    margen_solicitado='AUTO', notas='-'
):
    ...
    tasa_bs_cop_ingresada = float(tasa_bs_cop or 0)
    if tasa_bs_cop_ingresada > 0:
        q_bs_cop = tasa_bs_cop_ingresada
        fuente_bs_cop = 'NEGOCIADA_OPERACION'
    else:
        q_bs_cop = get_bscop_referencia(t)
        fuente_bs_cop = 'REFERENCIA_MERCADO'

    if q_bs_cop <= 0:
        return None, "No hay referencia BS→COP automática disponible. Ejecuta /tasas o introduce TASA-BS-COP en esta simulación."

    referido_activo = str(referido).strip().lower() not in ('no', '-', 'ninguno', '')
    delivery_cop = max(float(delivery_cop or 0), 0.0)
    otros_cop = max(float(otros_cop or 0), 0.0)
    costos_fijos_cop = delivery_cop + otros_cop
    ...
    niveles = calcular_niveles_tasa(
        ...
        delivery_cop=costos_fijos_cop, avance_corresponsal_cop=avance_corresponsal_cop,
        utilidad_minima_cop=UTILIDAD_MINIMA_COP,
    )

    # Política comercial: AUTO por monto o selección manual 7/5/3.
    politica = seleccionar_margen_clp_cop(float(monto_clp), margen_solicitado)

    # Compatibilidad defensiva: tasa_cliente_manual solo se conserva como legado interno.
    # La V1 visible usa MARGEN y no pide tasa manual al operador.
    if float(tasa_cliente_manual or 0) > 0:
        tasa_eval = float(tasa_cliente_manual)
        nivel_usado = 'TASA MANUAL LEGACY'
        objetivo = None
    else:
        objetivo = calcular_tasa_objetivo_con_piso(
            float(monto_clp), cop_potenciales, politica['margen'],
            referido=referido_activo, delivery_cop=costos_fijos_cop,
            avance_corresponsal_cop=avance_corresponsal_cop,
            utilidad_minima_cop=UTILIDAD_MINIMA_COP,
        )
        tasa_eval = objetivo['tasa']
        nivel_usado = politica['etiqueta']
    ...
    # BS→COP mínima necesaria para sostener el margen seleccionado.
    # Si el piso absoluto es más exigente, se transforma a margen efectivo.
    cop_cliente_obj = float(monto_clp) * float(tasa_eval)
    margen_efectivo_min = politica['margen']
    if cop_cliente_obj > 0:
        margen_efectivo_min = max(margen_efectivo_min, UTILIDAD_MINIMA_COP / cop_cliente_obj)

    q_objetivo_min = calcular_bs_cop_requerida(
        ciclo['bs_netos'], float(monto_clp), tasa_eval, margen_efectivo_min,
        referido=referido_activo, delivery_cop=costos_fijos_cop,
        avance_corresponsal_cop=avance_corresponsal_cop,
    )
    ...
    return {
        ...
        'monto_delivery':delivery_cop, 'otros_cop':otros_cop, 'costos_fijos_cop':costos_fijos_cop, 'delivery_activo':delivery_cop>0,
        'avance_corresponsal_cop':avance_corresponsal_cop, 'tasa_bs_cop':q_bs_cop,
        'fuente_tasa_bs_cop':fuente_bs_cop,
        'tasa_clp_usdt':tasa_clp_usdt, 'tasa_usdt_bs':tasa_usdt_bs,
        'ciclo':ciclo, 'cop_potenciales':cop_potenciales, 'niveles':niveles,
        'evaluacion':evaluacion, 'western_cmp':comparacion, 'bs_cop_requerida':inverso, 'q_objetivo_min':q_objetivo_min, 'margen_piso':margen_efectivo_min, 'politica_margen':politica, 'objetivo_tasa':objetivo,
    }, None
...
def msg_cotizacion_clp_cop_v1(r):
    ev = r['evaluacion']
    niv = r['niveles']
    pol = r.get('politica_margen') or {}
    obj = r.get('objetivo_tasa') or {}

    etiquetas = {
        'RECOMENDADA':'🟢 RECOMENDADA',
        'PREFERENCIAL':'🔵 PREFERENCIAL',
        'CAPTACION':'🟠 CAPTACIÓN',
        'FUERA_POLITICA':'⚠️ FUERA DE POLÍTICA',
        'EQUILIBRIO':'⚫ EQUILIBRIO',
        'PERDIDA':'🔴 PÉRDIDA'
    }

    q_ref = float(r.get('tasa_bs_cop', 0) or 0)
    q_obj = float(r.get('q_objetivo_min', 0) or 0)
    diferencia_q = q_ref - q_obj

    m = "🧮 *GSA — ANÁLISIS DEL GIRO*\n"
    m += "━━━━━━━━━━━━━━━━━━━━\n"
    m += "Operación: `CLP → COP`\n"
    m += f"Entrada: `{r['monto_entrada']:,.0f} CLP`\n"
    m += f"Política: `{'AUTO' if pol.get('modo') == 'AUTO' else 'MANUAL'} · {pol.get('etiqueta','')}`\n"
    if obj.get('piso_activo'):
        m += f"🛡️ Piso de utilidad activo: `{UTILIDAD_MINIMA_COP:,.0f} COP`\n"
    m += "\n"

    m += "*COSTOS ESPECIALES*\n"
    m += f"Referido: `{'SI — 3%' if r['referido_activo'] else 'NO'}`\n"
    m += f"Delivery: `{r.get('monto_delivery',0):,.0f} COP`\n"
    m += f"Corresponsal: `{r.get('avance_corresponsal_cop',0):,.0f} COP` adelantados\n"
    m += f"Otros: `{r.get('otros_cop',0):,.0f} COP`\n\n"

    m += "*COTIZACIÓN PARA EL CLIENTE*\n"
    m += f"Tasa aplicada: `{r['tasa_cliente']:.4f} COP/CLP`\n"
    m += f"Cliente recibe: `{ev['cop_cliente']:,.0f} COP`\n"
    m += f"Utilidad estimada: `{ev['utilidad_cop']:,.0f} COP`\n"
    m += f"Rentabilidad real: `{ev['rentabilidad_pct']:.2f}%`\n"
    m += f"Estado: *{etiquetas.get(ev['clasificacion'], ev['clasificacion'])}*\n\n"

    m += "*ALTERNATIVAS DE NEGOCIACIÓN*\n"
    m += f"🟢 7%: `{niv['recomendada']['tasa']:.4f}`\n"
    m += f"🔵 5%: `{niv['preferencial']['tasa']:.4f}`\n"
    m += f"🟠 3%: `{niv['captacion']['tasa']:.4f}`\n"
    m += f"⚫ Equilibrio: `{niv['equilibrio']['tasa']:.4f}`\n\n"

    m += "*COMPRA BS→COP*\n"
    m += f"Referencia mercado: `{q_ref:.4f} COP/Bs`\n"
    m += f"Mínimo necesario: `{q_obj:.4f} COP/Bs`\n"
    if diferencia_q >= 0:
        m += f"✅ Referencia actual supera el mínimo por `{diferencia_q:.4f}`.\n"
        m += "_Si consigues una tasa BS→COP mayor, mejoras la rentabilidad._"
    else:
        m += f"⚠️ Debes negociar al menos `{q_obj:.4f} COP/Bs` para sostener esta cotización."
...
def msg_cliente_clp_cop_v1(r):
    return (
        ...
        "━━━━━━━━━━━━━━━━━━━━\n"
        "🇨🇱 Chile → 🇨🇴 Colombia\n\n"
        f"💵 Monto: `{r['monto_entrada']:,.0f} CLP`\n"
        f"📊 Tasa: `{r['tasa_cliente']:.4f} COP/CLP`\n"
        f"💰 Recibes: `{r['monto_salida']:,.0f} COP`\n\n"
        "✅ *Monto final a recibir en Colombia*\n"
        "_Cotización sujeta a confirmación al momento de realizar el giro._"
    )
...
def main():
    ...
    # V1 activo: solo infraestructura de tasas, mercado y P2P.
    ...
    # LEGADO V6/V7 — desactivado en V1 simple:
    # loop_reporte_diario, loop_reporte_semanal, loop_gestor_capital, loop_csv
