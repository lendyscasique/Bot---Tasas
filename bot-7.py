...
def obtener_tramos_clp_cop(d, persistir=True):
    """Tramo 1 dinámico; siguientes +0.05. Baja intradía por protección, no sube hasta el día siguiente."""
    raw = calcular_base_clp_cop_dinamica(d)
    if raw <= 0:
        return {"base":0.0,"candidata":0.0,"estado":"SIN_DATO","tramos":[]}
    candidata = round(raw, 2)
    publicada = candidata
    estado = "NUEVA"
    ...
    if persistir:
        try:
            ...
            if fecha != hoy or previa <= 0:
                ...
                set_config("clp_cop_base_publicada", f"{publicada:.2f}")
                estado = "APERTURA"
            ...
            # Permite pruebas/importación antes de init_db; en producción
            # init_db crea la tabla config y la ancla diaria queda persistida.
            publicada = candidata
    ...
    return {
        ...
        "estado": estado,
        "tramos": [
            (25000, round(publicada,2)),
            (50000, round(publicada+0.05,2)),
            (150000, round(publicada+0.10,2)),
            (float("inf"), round(publicada+0.15,2)),
        ],
    }
...
def tasas_bs_cop_comerciales(d):
    ...
    if limite <= 0:
        return None
    return {
        "compra_bs": round(limite * (1 - FEE_COP_BS), 4),
        "venta_bs": round(limite * (1 + FEE_BS_COP), 4),
    }
...
def tasas_usd_cop_comerciales(d):
    """Orientación Binance confirmada por operación GSA.

    Regla:
      COP → USDT = lado VENDE
      USDT → COP = lado COMPRA

    Para USD físico:
      Compra USD (GSA recibe USD y entrega COP)  -> referencia USDT→COP -> COMPRA
      Venta USD  (GSA recibe COP y entrega USD)  -> referencia COP→USDT -> VENDE

    Estas tasas Binance son referencia económica/guardrail. Para cotización comercial
    USD físico↔COP, el mercado físico de Cúcuta sigue siendo el price-former principal
    cuando esté disponible.
    """
    lado_vende = float(d.get("cop_compra", 0) or 0)  # legacy: corresponde a VENDE
    lado_compra = float(d.get("cop_venta", 0) or 0) # legacy: corresponde a COMPRA
    if lado_vende <= 0 or lado_compra <= 0:
        return None
    return {
        "compra_usd_ref": round(lado_compra, 2),  # USD→...→COP
        "venta_usd_ref": round(lado_vende, 2),   # COP→...→USD
    }

def construir_mensaje_tasas_v1(d, es_especial=False):
    """Pizarra comercial: solo tasas de servicios + referencias compactas."""
    ...
    try:
        ...
        if q.get("estado") != ESTADO_BLOQUEADO:
            m += f"🇨🇱 CLP → 🇻🇪 Bs   `{q['tasa_comercial']:.4f} Bs/CLP`\n"
    except Exception:
        pass
    if limite_clp_bs > 0:
        m += f"🇻🇪 Bs → 🇨🇱 CLP   `{limite_clp_bs*1.10:.4f} Bs/CLP`\n"
    ...
    if info["tramos"]:
        ...
        m += "\n🇨🇱 *CLP → 🇨🇴 COP*\n"
        m += f"• Hasta 25.000       → `{t1:.2f}`\n"
        m += f"• 25.001 – 50.000    → `{t2:.2f}`\n"
        m += f"• 50.001 – 150.000   → `{t3:.2f}`\n"
        m += f"• Desde 150.001      → `{t4:.2f}`\n"
    ...
    if d.get("western"):
        try:
            ...
            if q.get("estado") != ESTADO_BLOQUEADO:
                m += f"🇨🇴 COP → 🇨🇱 CLP   `{q['tasa_comercial']:.4f} COP/CLP`\n"
        except Exception:
            pass
    ...
    if d.get("dolar_obs"):
        ...
        m += "\n💵🇨🇱 *USD / Pesos Chilenos*\n"
        m += f"• Venta USD           → `{obs+50:.2f} CLP/USD`\n"
        m += f"• Compra USD          → `{obs-50:.2f} CLP/USD`\n"

    # USDT ↔ CLP
    clp_compra = float(d.get("clp_compra", 0) or 0)
    clp_venta = float(d.get("clp_venta", 0) or 0)
    if clp_compra > 0 or clp_venta > 0:
        m += "\n💎🇨🇱 *USDT / Pesos Chilenos*\n"
        if clp_compra > 0:
            m += f"• Venta USDT          → `{clp_compra+20:.2f} CLP/USDT`\n"
        if clp_venta > 0:
            m += f"• Compra USDT         → `{clp_venta-20:.2f} CLP/USDT`\n"

    # Referencias compactas
    refs=[]
    if d.get("bcv_usd"): refs.append(f"🇺🇸 BCV USD `{fmt(d['bcv_usd'])}`")
    if d.get("bcv_eur"): refs.append(f"🇪🇺 BCV EUR `{fmt(d['bcv_eur'])}`")
    if d.get("trm"): refs.append(f"🇨🇴 TRM `{fmt(d['trm'])}`")
    if d.get("dolar_obs"): refs.append(f"🇨🇱 Obs. `{fmt(d['dolar_obs'])}`")
    if d.get("western"): refs.append(f"🌎 Western `{fmt(d['western'],4)}`")
    if refs:
        m += "\n━━━━━━━━━━━━━━━━━━━━\n📌 *Referencias*\n" + "\n".join(refs)
    return m
...
def procesar(chat_id, texto):
    ...
    if MODO_V1_SIMPLE and cmd.startswith('/') and cmd not in COMANDOS_V1:
        send(chat_id,
             "ℹ️ *Comando fuera del alcance de GSA V1.*\n\n"
             "El Bot ahora se concentra en tasas, mercado, pricing y análisis P2P. "
             "Operaciones, inventario, saldos, resultados y pendientes se gestionan en Excel.\n\n"
             "Usa /ayuda para ver los comandos activos.")
        return
        ...
        if len(partes)>=2:
            try:
                ...
                send(chat_id,
                     ...
                     f"Referencia: `{western_rate:.4f} COP/CLP`\n\n"
                     f"💸 *Servicio Western GSA*\n"
                     f"Nominal ejemplo: `{nominal_ejemplo:,.0f} COP`\n"
                     f"Comisión GSA (5%): `{r_w['comision_gsa']:,.0f} COP`\n"
                     f"Cliente recibe: `{r_w['monto_cliente']:,.0f} COP`\n\n"
                     "_El 5% se descuenta del nominal; no se agrega por encima._\n"
                     "_Los costos reales de ejecución se consideran aparte cuando existan._\n\n"
                     "_Western sigue siendo benchmark y no determina directamente la tasa comercial CLP→COP._")
            except Exception as e:
    ...
    # ── SISTEMA ──
        ...
        send(chat_id,"""🤖 *GSA CAMBIOS — ASISTENTE DE TASAS V1*

*📊 TASAS*
/tasas — Panel general de mercado y tasas GSA
/western TASA — Actualizar benchmark Western
/limites — Mesa rápida de negociación por monto

*📡 MERCADO*
/mercado — Lectura del mercado en vivo
/patron — Tendencias e histórico
  `/patron bs` | `/patron hoy` | `/patron semana`

*🧮 SIMULADOR*
/simular — Cotizar una operación especial

*📊 ANÁLISIS P2P*
/profundidad — Profundidad y liquidez de mercado
/mianuncio — Estado de tu anuncio P2P

_El Bot analiza y ayuda a decidir. Las operaciones reales, inventario, saldos y resultados se registran en Excel._""")
...
def msg_limites_clp_cop_v1():
    """Mesa rápida CLP→COP para operaciones estándar, usando mercado actual.

    Se muestran tres montos de referencia para probar la experiencia V1.
    No sustituye /simular cuando existen costos especiales.
    """
    t = get_ultima_tasa() or {}
    tasa_clp_usdt = t.get('clp_compra', 0) or t.get('dolar_obs', 0) or 0
    tasa_usdt_bs = max(t.get('ban_bs_venta', 0) or 0, t.get('mer_bs_venta', 0) or 0)
    q_ref = get_bscop_referencia(t)

    if tasa_clp_usdt <= 0 or tasa_usdt_bs <= 0 or q_ref <= 0:
        return "⚠️ No hay mercado suficiente para calcular límites. Ejecuta /tasas primero."
    ...
    def fila(monto):
        ...
        capacidad = calcular_capacidad_cop(ciclo['bs_netos'], q_ref)

        # En operación estándar no hay referido, delivery ni corresponsal.
        # Para cada nivel protegemos también la utilidad mínima absoluta.
        def tasa_con_piso(margen):
            p_pct = capacidad / (1.0 + margen)
            p_abs = capacidad - UTILIDAD_MINIMA_COP
            p = max(0.0, min(p_pct, p_abs))
            return p / float(monto) if monto else 0.0

        return {
            'monto': monto,
            'r7': tasa_con_piso(MARGEN_RECOMENDADO),
            'r5': tasa_con_piso(MARGEN_PREFERENCIAL),
            'r3': tasa_con_piso(MARGEN_CAPTACION),
            'max': max(0.0, (capacidad - UTILIDAD_MINIMA_COP) / float(monto)),
        }

    filas = [fila(50000), fila(100000), fila(250000)]

    m = "📐 *LÍMITES COMERCIALES GSA*\n"
    m += "━━━━━━━━━━━━━━━━━━━━\n"
    m += "*CLP → COP · operación estándar*\n"
    m += "_Sin referido · sin delivery · sin corresponsal · sin otros gastos_\n\n"
    m += f"BS→COP ref. mercado: `{q_ref:.4f} COP/Bs`\n"
    m += f"Utilidad mínima protegida: `{UTILIDAD_MINIMA_COP:,.0f} COP`\n\n"

    etiquetas = ["Hasta 50.000 CLP · AUTO 7%", "50.001–100.000 CLP · AUTO 7%", "Más de 100.000 CLP · AUTO 5%"]
    referencias = ["50.000", "100.000", "250.000"]
    for et, ref, f in zip(etiquetas, referencias, filas):
        m += f"*{et}* _(ref. {ref})_\n"
        m += f"🟢 7% Recomendada: `{f['r7']:.4f}`\n"
        m += f"🔵 5% Preferencial: `{f['r5']:.4f}`\n"
        m += f"🟠 3% Captación: `{f['r3']:.4f}`\n"
        m += f"🛡️ Tope con utilidad mínima: `{f['max']:.4f}`\n\n"

    m += "_Si la operación tiene costos especiales, usa /simular._\n"
    m += "_Esta V1 prueba CLP→COP. Las demás rutas entrarán con el motor universal._"
    return m
...
def calcular_simulacion_motor_v1(tipo_op, monto, campos=None, tasas=None):
    ...
    if tipo == "CLP→COP":
        ...
        return cotizar_clp_cop(
            ...
            costos_pendientes=_campo_num(campos, "DELIVERY", 0) + _campo_num(campos, "OTROS", 0),
            tasa_cliente=tasa_manual if tasa_manual > 0 else tasa_publicada,
        )
...
def msg_simular_ayuda():
    """Plantilla universal del Motor V1."""
    return (
        "🧮 *SIMULAR — MOTOR GSA V1*\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "Formato base:\n\n"
        "`/simular`\n"
        "`TIPO: CLP-COP`\n"
        ...
        "`DELIVERY: 0`\n"
        "`OTROS: 0`\n\n"
        "*18 operaciones soportadas*\n"
        "`CLP-BS · BS-CLP · CLP-COP · COP-CLP`\n"
        "`CLP-USD · USD-CLP · CLP-USDT · USDT-CLP`\n"
        "`WESTERN-COP · COP-BS · BS-COP · USD-BS`\n"
        "`BS-USD · USD-COP · COP-USD · USDT-COP`\n"
        "`USDT-USD · USD-USDT`\n\n"
        "*Campos solo cuando apliquen*\n"
        "`PRECIO-MAKER`, `PRECIO-TAKER`, `TASA-BS-COP`, `TASA-DIRECTA`,\n"
        "`COSTO-REPOSICION`, `VALOR-ECONOMICO`, `MERCADO-COMPRA`,\n"
        "`MERCADO-VENTA`, `BANESCO-OTRO-BANCO`, `NECESIDAD-USD`, `COSTO-USDT`.\n\n"
        "_El Bot usa automáticamente los datos de mercado que ya conoce y solo exige datos manuales cuando no dispone de ellos._"
    )
