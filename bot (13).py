...
def procesar(chat_id, texto):
    ...
    if cmd=='/tasas':
        ...
        # Límites REFERENCIALES legacy:
        # CLP/BS = mercado Binance/observado
        # CLP/COP = Western ajustado por FEE_WU
        # BS/COP = CLP/COP / CLP/BS
        # Estos límites NO forman la nueva tasa comercial CLP→COP del Motor Pricing V1.
        t = dict(ultimo_datos or get_ultima_tasa() or {})

        limite_clp_bs = t.get('limite_clp_bs')
        limite_clp_cop = t.get('limite_clp_cop')
        limite_bs_cop = t.get('limite_bs_cop')

        # Si Western no venía en memoria/último snapshot, reconstruir los dos
        # límites derivados usando la última Western persistida.
        try:
            western_guardado = float(get_config('western_rate', '0') or 0)
        except Exception:
            western_guardado = 0

        if western_guardado > 0:
            limite_clp_cop = western_guardado * (1 - FEE_WU)
            if limite_clp_bs:
        ...
        m = "📐 *LÍMITES REFERENCIALES*\n"
        m += f"CLP/BS:  `{_f_lim(limite_clp_bs, 6)}`\n"
        m += f"CLP/COP: `{_f_lim(limite_clp_cop, 4)}`\n"
        m += f"BS/COP:  `{_f_lim(limite_bs_cop, 4)}`\n\n"
        m += "_Western se usa aquí solo para la referencia CLP/COP y BS/COP. "
        m += "No determina la tasa comercial del Motor Pricing V1._"
        send(chat_id,m)
...
/limites — Ver límites referenciales
...
*🧮 SIMULADOR*
/simular — Cotizar y analizar una operación
...
def msg_simular_ayuda():
    ...
    m += "━━━━━━━━━━━━━━━━━━━━\n\n"
    m += "Para *CLP→COP*, la referencia BS→COP se calcula automáticamente.\n"
    m += "Si ya negociaste una tasa específica para esa operación, escríbela en `TASA-BS-COP`.\n\n"
    ...
    m += "`NOTAS: -`\n\n"
    m += "*CLP→COP V1:*\n"
    m += "• `TASA-BS-COP: 0` usa la referencia automática de mercado.\n"
    m += "• Si negociaste una tasa distinta para esta operación, colócala allí; no se guarda globalmente.\n"
...
def msg_cotizacion_clp_cop_v1(r):
    ...
    if r.get('fuente_tasa_bs_cop') == 'NEGOCIADA_OPERACION':
        m+=f"BS→COP operación: `{r['tasa_bs_cop']:.4f}` ✅ _tasa ingresada para esta operación_\n"
    else:
        m+=f"BS→COP referencia: `{r['tasa_bs_cop']:.4f}` ⚠️ _referencia automática; no negociada_\n"
