# GSA CAMBIOS BOT — V1.0.8 / BCV simplificado
"""
...
def get_bcv_info():
    """BCV SOLO REFERENCIAL.

    Devuelve únicamente la publicación BCV disponible actualmente:
      USD, EUR y fecha valor.

    No intenta reconstruir tasa anterior ni separar vigente/próxima jornada.
    No interviene en cálculos GSA.

    Fuente rápida: DolarAPI/BCV.
    Si falla, usa el último valor válido guardado para evitar bloquear /tasas.
    """
    headers = {"User-Agent": "Mozilla/5.0 GSA-Cambios/1.0"}
    ...
    try:
        ...
        for item in data if isinstance(data, list) else []:
            ...
            fecha_raw = fecha_raw or str(
                item.get("fechaActualizacion")
                or item.get("fecha")
                or item.get("fechaValor")
                or ""
            )
        ...
        print(f"[BCV] consulta rápida falló: {e}")

    # Fallback local: no hace una segunda consulta web.
    usd_prev = _leer_tasa_config("bcv_actual_usd")
    eur_prev = _leer_tasa_config("bcv_actual_eur")

    if usd_prev.get("valor") and eur_prev.get("valor"):
        return {
            "usd": usd_prev.get("valor"),
            "eur": eur_prev.get("valor"),
            "fecha": usd_prev.get("fecha") or eur_prev.get("fecha") or "",
            "fuente": "Último BCV disponible",
            "respaldo": True,
        }
...
def consultar_y_guardar(western_rate=None):
    ...
    # Guardar en DB
    if USE_SUPABASE:
        _meta_no_db = {'mejor_banco','bcv_fecha','bcv_fuente','bcv_respaldo','trm_fecha','trm_vigencia','trm_fuente','trm_respaldo'}
...
def construir_mensaje(d, es_especial=False):
    ...
    if d.get('trm'):
        ...
        if d.get('trm_respaldo'):
            m += "      ⚠️ _Último valor disponible_\n"
        m += "\n"
    else:
        m += "🇨🇴 *TRM*\n      _No disponible_ ⚠️\n\n"

    if d.get('bcv_usd') or d.get('bcv_eur'):
        m += "🇻🇪 *BCV — PUBLICACIÓN ACTUAL*\n"
        if d.get('bcv_usd'):
