# GSA CAMBIOS BOT — V1.0.7 / Indicadores oficiales por vigencia
"""
...
def _numero_latam(valor):
    """Convierte 3.126,08 / 3,126.08 / 814,69080000 a float."""
    if valor is None:
        return None
    s = str(valor).strip().replace("\xa0", "").replace(" ", "")
    # Si aparecen punto y coma, el último separador define decimales.
    if "." in s and "," in s:
        if s.rfind(",") > s.rfind("."):
            s = s.replace(".", "").replace(",", ".")
        else:
...
def _guardar_tasa_config(prefijo, valor, fecha="", fuente=""):
    if valor:
        set_config(f"{prefijo}_valor", str(valor))
        set_config(f"{prefijo}_fecha", str(fecha or ""))
        set_config(f"{prefijo}_fuente", str(fuente or ""))


def _leer_tasa_config(prefijo):
    try:
        valor = float(get_config(f"{prefijo}_valor", "0") or 0)
    except Exception:
        valor = 0.0
    return {
        "valor": valor or None,
        "fecha": get_config(f"{prefijo}_fecha", "") or "",
        "fuente": get_config(f"{prefijo}_fuente", "") or "",
        "respaldo": True,
    }
...
def get_trm_info():
    """TRM informativa y validada por vigencia.
    Prioridad:
      1) Banco de la República (página del indicador, valor + fecha)
      2) Superfinanciera (valor + rango de vigencia)
      3) Último valor válido persistido, marcado como respaldo

    Importante: TRM es SOLO REFERENCIAL y no alimenta cálculos GSA.
    """
    headers = {"User-Agent": "Mozilla/5.0 GSA-Cambios/1.0"}

    # 1) Banco de la República: valor y fecha explícitos.
    try:
        ...
        # Busca valor con formato colombiano: 3.126,08
        m_val = re.search(r"([0-9]{1,2}\.[0-9]{3},[0-9]{2})", html)
        # Busca fecha dd/mm/yyyy asociada a la página del indicador
        m_fecha = re.search(r"([0-3]?\d/[01]?\d/[0-9]{4})", html)

        if m_val:
            valor = _numero_latam(m_val.group(1))
            fecha = m_fecha.group(1) if m_fecha else ""
            if valor and 1000 < valor < 10000:
                info = {
                    "valor": valor,
                    "fecha": fecha,
                    "vigencia": fecha,
                ...
                _guardar_tasa_config("trm_oficial", valor, fecha, "Banco de la República")
                return info
    except Exception as e:
        print(f"[TRM] Banco de la República falló: {e}")

    # 2) Superintendencia Financiera: valor + rango de vigencia
    try:
        url = "https://www.superfinanciera.gov.co/CargaDriver/"
        r = requests.get(url, headers=headers, timeout=15)
        r.raise_for_status()
        html = re.sub(r"\s+", " ", r.text)

        # Ejemplo: TRM | COP | 3,126.08 | 5/Sep/2026 - 8/Sep/2026
        m = re.search(
            r"TRM.{0,250}?COP.{0,250}?([0-9]{1,3}(?:,[0-9]{3})*\.[0-9]{2}).{0,250}?([0-9]{1,2}/[A-Za-z]{3}/[0-9]{4}\s*-\s*[0-9]{1,2}/[A-Za-z]{3}/[0-9]{4})",
            html, re.I | re.S
        )
        if m:
            valor = _numero_latam(m.group(1))
            vigencia = m.group(2)
            if valor and 1000 < valor < 10000:
                info = {
                    "valor": valor,
                ...
                _guardar_tasa_config("trm_oficial", valor, vigencia, "Superfinanciera")
                return info
    except Exception as e:
        print(f"[TRM] Superfinanciera falló: {e}")

    # 3) Último valor válido guardado
    ultimo = _leer_tasa_config("trm_oficial")
    if ultimo.get("valor"):
        return {
            "valor": ultimo.get("valor"),
            "fecha": ultimo.get("fecha") or "",
            "vigencia": ultimo.get("fecha") or "",
            "fuente": (ultimo.get("fuente") or "Último valor válido") + " · respaldo",
            "respaldo": True,
        }

    return {
        "valor": None, "fecha": "", "vigencia": "",
        "fuente": "No disponible", "respaldo": True
    }


def get_trm():
    return get_trm_info().get("valor")


def _extraer_bcv_html(html):
    """Intenta extraer USD, EUR y Fecha Valor del HTML oficial BCV."""
    limpio = re.sub(r"\s+", " ", html or "")
    usd = eur = None

    # Patrones tolerantes a la estructura histórica del portal BCV.
    patrones = {
        "usd": [
            r"USD.{0,350}?([0-9]{1,4},[0-9]{4,12})",
            r"d[oó]lar.{0,350}?([0-9]{1,4},[0-9]{4,12})",
        ],
        "eur": [
            r"EUR.{0,350}?([0-9]{1,4},[0-9]{4,12})",
            r"euro.{0,350}?([0-9]{1,4},[0-9]{4,12})",
        ],
    }

    for p in patrones["usd"]:
        m = re.search(p, limpio, re.I | re.S)
        if m:
            v = _numero_latam(m.group(1))
            if v and 1 < v < 100000:
                usd = v
                break

    for p in patrones["eur"]:
        m = re.search(p, limpio, re.I | re.S)
        if m:
            v = _numero_latam(m.group(1))
            if v and 1 < v < 100000:
    ...
    fecha = ""
    mf = re.search(r"Fecha\s*Valor\s*:?\s*([^<]{5,60}?)(?:<|$)", html or "", re.I | re.S)
    if mf:
        fecha = re.sub(r"\s+", " ", mf.group(1)).strip()

    return usd, eur, fecha



def _parse_fecha_bcv(fecha_raw):
    """Normaliza fechas comunes del BCV/DolarAPI a DD/MM/YYYY si es posible."""
    if not fecha_raw:
        return ""
...
def get_bcv_info():
    """BCV informativo con separación de vigente y próxima jornada.
    BCV NO alimenta cálculos GSA.

    Estructura devuelta:
      actual: {usd, eur, fecha}
      proximo: {usd, eur, fecha} o None

    Fuente principal práctica:
      - DolarAPI/BCV, porque suele exponer fecha de actualización.
    Fallback:
      - Portal oficial BCV
      - Último valor válido persistido
    """
    headers = {"User-Agent": "Mozilla/5.0 GSA-Cambios/1.0"}

    # Hora/fecha local del servidor. Solo se usa para clasificar vigente vs próxima.
    try:
    ...
    try:
        ...
        for item in data if isinstance(data, list) else []:
            moneda = str(item.get("moneda", "")).lower()
            valor = item.get("promedio") or item.get("precio") or item.get("venta")
            if moneda == "usd":
                usd = _numero_latam(valor)
            elif moneda == "eur":
                eur = _numero_latam(valor)
            fecha_raw = fecha_raw or str(
                item.get("fechaActualizacion") or
                item.get("fecha") or
                item.get("fechaValor") or
                ""
            )

        fecha = _parse_fecha_bcv(fecha_raw)

        # Intentar clasificar por fecha.
        fecha_dt = None
        if fecha:
            m = re.search(r"([0-3]?\d)/([01]?\d)/(20\d{2})", fecha)
            if m:
                from datetime import date
                fecha_dt = date(int(m.group(3)), int(m.group(2)), int(m.group(1)))

        if usd and eur:
            payload = {"usd": usd, "eur": eur, "fecha": fecha}
            _guardar_tasa_config("bcv_ultimo", usd, fecha, "DolarAPI/BCV")

            # Si la fecha publicada es futura, NO la llamamos vigente.
            if hoy and fecha_dt and fecha_dt > hoy:
                # Recuperar último vigente guardado por separado, si existe.
                actual_usd = _leer_tasa_config("bcv_actual_usd")
                actual_eur = _leer_tasa_config("bcv_actual_eur")

                actual = None
                if actual_usd.get("valor") and actual_eur.get("valor"):
                    actual = {
                        "usd": actual_usd.get("valor"),
                        "eur": actual_eur.get("valor"),
                        "fecha": actual_usd.get("fecha") or actual_eur.get("fecha") or "",
                    }
            ...
            # Es vigente para hoy o no trae fecha futura.
            _guardar_tasa_config("bcv_actual_usd", usd, fecha, "DolarAPI/BCV")
            _guardar_tasa_config("bcv_actual_eur", eur, fecha, "DolarAPI/BCV")
            return {
                "actual": payload,
                "proximo": None,
                "fuente": "DolarAPI/BCV",
        ...
        print(f"[BCV] DolarAPI falló: {e}")

    # 2) Portal oficial BCV
    try:
        r = requests.get(
        ...
        r.raise_for_status()
        usd, eur, fecha = _extraer_bcv_html(r.text)
        fecha = _parse_fecha_bcv(fecha)

        if usd and eur:
            _guardar_tasa_config("bcv_actual_usd", usd, fecha, "BCV")
            _guardar_tasa_config("bcv_actual_eur", eur, fecha, "BCV")
            return {
                "actual": {"usd": usd, "eur": eur, "fecha": fecha},
                "proximo": None,
                "fuente": "BCV",
                "respaldo": False,
            }
    except Exception as e:
        print(f"[BCV] portal oficial falló: {e}")

    # 3) Último vigente persistido
    actual_usd = _leer_tasa_config("bcv_actual_usd")
    actual_eur = _leer_tasa_config("bcv_actual_eur")
    if actual_usd.get("valor") and actual_eur.get("valor"):
        return {
            ...
            "proximo": None,
            "fuente": "Último BCV vigente · respaldo",
            "respaldo": True,
        }
...
def get_bcv():
    ...
    return actual.get("usd"), actual.get("eur")
...
def consultar_y_guardar(western_rate=None):
    ...
    trm = trm_info.get('valor')
    # Compatibilidad con lógica previa
    if trm:
        set_config('trm_hoy', str(trm))

    bcv_info = get_bcv_info()
    bcv_actual = bcv_info.get('actual') or {}
    bcv_proximo = bcv_info.get('proximo') or {}
    bcv_usd = bcv_actual.get('usd')
    bcv_eur = bcv_actual.get('eur')
    ...
    datos = {
        ...
        'bcv_fecha':bcv_actual.get('fecha'),'bcv_fuente':bcv_info.get('fuente'),
        'bcv_respaldo':bcv_info.get('respaldo',False),
        'bcv_prox_usd':bcv_proximo.get('usd'),'bcv_prox_eur':bcv_proximo.get('eur'),
        'bcv_prox_fecha':bcv_proximo.get('fecha'),
        'trm_fecha':trm_info.get('fecha'),'trm_vigencia':trm_info.get('vigencia'),
        'trm_fuente':trm_info.get('fuente'),
        'trm_respaldo':trm_info.get('respaldo',False),
    ...
    # Guardar en DB
    if USE_SUPABASE:
        _meta_no_db = {'mejor_banco','bcv_fecha','bcv_fuente','bcv_respaldo','bcv_prox_usd','bcv_prox_eur','bcv_prox_fecha','trm_fecha','trm_vigencia','trm_fuente','trm_respaldo'}
        supa_insert('tasas', {k: v for k, v in datos.items() if k not in _meta_no_db and v is not None})
    conn = get_conn(); c = conn.cursor()
...
def construir_mensaje(d, es_especial=False):
    ...
    # TASAS OFICIALES — SOLO REFERENCIA
    m += "🌎 *TASAS OFICIALES — REFERENCIA*\n\n"

    if d.get('trm'):
        m += f"🇨🇴 *TRM VIGENTE*\n      `{fmt(d['trm'])} COP/USD`\n"
        if d.get('trm_vigencia'):
            m += f"      Vigencia: `{d['trm_vigencia']}`\n"
        elif d.get('trm_fecha'):
            m += f"      Fecha: `{d['trm_fecha']}`\n"
        if d.get('trm_fuente'):
            suf = " ⚠️ respaldo" if d.get('trm_respaldo') else ""
            m += f"      Fuente: _{d['trm_fuente']}{suf}_\n"
        m += "\n"
    else:
        m += "🇨🇴 *TRM*\n      _No disponible_ ⚠️\n\n"

    if d.get('bcv_usd') or d.get('bcv_eur'):
        m += "🇻🇪 *BCV VIGENTE*\n"
        if d.get('bcv_usd'):
            m += f"      USD: `{fmt(d['bcv_usd'])} Bs/USD`\n"
        if d.get('bcv_eur'):
            m += f"      EUR: `{fmt(d['bcv_eur'])} Bs/EUR`\n"
        if d.get('bcv_fecha'):
            m += f"      Fecha valor: `{d['bcv_fecha']}`\n"
        if d.get('bcv_fuente'):
            suf = " ⚠️ respaldo" if d.get('bcv_respaldo') else ""
            m += f"      Fuente: _{d['bcv_fuente']}{suf}_\n"
        m += "\n"

    if d.get('bcv_prox_usd') or d.get('bcv_prox_eur'):
        m += "📅 *BCV PRÓXIMA JORNADA*\n"
        if d.get('bcv_prox_usd'):
            m += f"      USD: `{fmt(d['bcv_prox_usd'])} Bs/USD`\n"
        if d.get('bcv_prox_eur'):
            m += f"      EUR: `{fmt(d['bcv_prox_eur'])} Bs/EUR`\n"
        if d.get('bcv_prox_fecha'):
            m += f"      Vigente desde: `{d['bcv_prox_fecha']}`\n"
        m += "      _Solo informativa hasta su fecha de vigencia._\n\n"

    if not (d.get('bcv_usd') or d.get('bcv_eur') or d.get('bcv_prox_usd') or d.get('bcv_prox_eur')):
        m += "🇻🇪 *BCV*\n      _No disponible_ ⚠️\n\n"

    m += "_ℹ️ TRM y BCV son referencias informativas y no intervienen en los cálculos GSA._\n\n"
        ...
        if len(partes)>=2:
            try:
                ...
                send(chat_id,
                     ...
                     f"`{western_rate:.4f} COP/CLP`\n\n"
                     "_Se usa como benchmark y para límites referenciales. "
                     "No determina directamente la tasa comercial CLP→COP._")
            except: send(chat_id,"Uso: /western 3.3628")
        else: send(chat_id,"Uso: `/western 3.3628`")
    ...
    # ── SIMULAR ──
        ...
        if len(lineas) <= 1:
            ...
            try:
                ...
                delivery_cop = _float_monto(campos.get('DELIVERY','0'))
                avance_corresponsal = _float_monto(campos.get('CORRESPONSAL','0'))
                otros_cop = _float_monto(campos.get('OTROS','0'))
                margen_solicitado = campos.get('MARGEN','AUTO')

                # Valores legacy conservados únicamente para las rutas antiguas.
                cliente = 'Cliente'
                metodo = 'Efectivo'
                entrega = 'Efectivo'
                tipo_corresp = 'No'
                titular_corresp = ''
                com_corresp_pct = 0.0
                com_ref_pct = 0.03 if referido.lower() not in ('no','-','ninguno','') and tipo_op == 'CLP→COP' else 0.0
                delivery_val = 'Si' if delivery_cop > 0 else 'No'
                monto_delivery = delivery_cop
                notas = '-'
                ...
                if tipo_op not in TIPOS_OP:
                    ...
                    resultado, error = calcular_cotizacion_clp_cop_v1(
                        ...
                        tasa_bs_cop=0, tasa_cliente_manual=0,
                        referido=referido, delivery_cop=delivery_cop,
                        avance_corresponsal_cop=avance_corresponsal,
                        otros_cop=otros_cop, margen_solicitado=margen_solicitado,
                        notas=notas)
                    if error:
...
def msg_simular_ayuda():
    """Plantilla mínima del cotizador especial. V1 operativo: CLP→COP."""
    return (
        "🧮 *SIMULAR OPERACIÓN ESPECIAL*\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "Copia este bloque, cambia solo lo necesario y vuelve a enviarlo:\n\n"
        "`/simular`\n"
        "`TIPO: CLP-COP`\n"
        "`MONTO: 300000`\n"
        "`REFERIDO: NO`\n"
        ...
        "`MARGEN: AUTO`\n\n"
        "*MARGEN*\n"
        "• `AUTO`: hasta 100.000 CLP usa 7%; desde 100.001 CLP usa 5%.\n"
        "• También puedes escribir `7`, `5` o `3` para elegirlo manualmente.\n"
        "• El Bot protege siempre la utilidad mínima absoluta de 10.000 COP.\n\n"
        "*COSTOS*\n"
        "• `REFERIDO`: SI o NO. El Bot aplica 3% automáticamente.\n"
        "• `DELIVERY`: costo que paga GSA en COP; 0 si no aplica.\n"
        "• `CORRESPONSAL`: COP realmente adelantados; el Bot aplica 5%.\n"
        "• `OTROS`: otros costos de GSA en COP; 0 si no aplica.\n\n"
        "El Bot calcula mercado, fees, tasa al cliente y la tasa BS→COP mínima necesaria.\n"
        "_Por ahora el nuevo motor está validado para CLP→COP._"
    )
